...
맵은 캐러셀 위치 계산용 경량 슬라이드는 유지하되 현재 viewport와 이동 버퍼를 합쳐
최대 5개 다이만 내부 콘텐츠를 마운트한다. `mountedBeds`, `withinVisibleBedLimit`와 `domGrowth`는 전후 비교를
위한 관측값이다. 최종 선택 데이터 준비 여부와 실제 드래그의 선택 보존 여부는 계속
검증한다. 범위별 통합 이력 요청과 캐시가 적용되므로 재방문·단순 스와이프의 요청 수는
0일 수 있다.
