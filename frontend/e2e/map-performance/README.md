...
제품 코드는 변경하거나 계측하지 않는다. Playwright가 기존 접근성 이름과 로딩 문구,
work-history 네트워크 응답을 관찰하고 응답 완료 후 두 animation frame이 지난 시점을
렌더 완료로 측정한다.

맵은 현재 전체 다이 슬라이드를 DOM에 유지한다. 따라서 `withinVisibleBedLimit`는
현재 구조의 관측값이며 baseline 실패 조건으로 사용하지 않는다. 대신 연속 이동 전후
DOM 증가, 최종 선택 데이터 준비 여부, 실제 드래그의 선택 보존 여부를 검증한다.
