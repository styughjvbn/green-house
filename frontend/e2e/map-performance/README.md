...
맵은 캐러셀 위치 계산용 경량 슬라이드는 유지하되 현재 viewport와 앞뒤 각각 표시
개수만큼의 이동 버퍼만 내부 콘텐츠를 마운트한다. `mountedBeds`, `withinVisibleBedLimit`와 `domGrowth`는 전후 비교를
위한 관측값이다. 최종 선택 데이터 준비 여부와 실제 드래그의 선택 보존 여부는 계속
검증한다. 범위별 통합 이력은 페이지 응답의 `totalElements`와 첫 페이지 표본을 검증하고,
난 묶음 상세는 모든 페이지를 순회해 직접·전파 이력 비율과 중복을 확인한다. 캐시가
적용되므로 재방문·단순 스와이프의 요청 수는 0일 수 있다.
