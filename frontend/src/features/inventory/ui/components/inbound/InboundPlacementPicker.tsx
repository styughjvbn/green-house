...
export type InboundPlacementSelection = {
  bedZoneId: number;
  startCell: number;
  endCell: number;
...
function InboundPlacementPickerDialog({
  houses,
  initialValue,
  onClose,
  onSelect,
  ...
  houses: House[];
  initialValue: InboundPlacementSelection | null;
  onClose: () => void;
  onSelect: (value: InboundPlacementSelection) => void;
}) {
  ...
  const occupiedCells = useMemo(
    () => buildOccupiedCells(selected?.zone.orchidGroups ?? []),
    [selected],
  );
      ...
      >
        <header className="flex items-center justify-between border-b border-[#e1e7df] px-4 py-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-[#66736a]">
              구역을 고른 뒤 입고 난 묶음이 차지할 시작 칸과 끝 칸을 지정하세요.
            </p>
          </div>
          <button
        ...
        <footer className="flex items-center justify-end gap-2 border-t border-[#e1e7df] px-4 py-3">
          ...
          <button
            ...
            disabled={!selected || hasOverlap}
            type="button"
            onClick={() => {
              if (!selected || hasOverlap) return;
              onSelect({
...
function ZonePreview({
  ...
  const occupiedCells = buildOccupiedCells(zone.orchidGroups);
...
function buildOccupiedCells(orchidGroups: OrchidGroup[]) {
  const cells = new Set<number>();
