  ...
  return (
    <form className="rounded-md border border-[#d7ddd4] bg-white p-4 shadow-sm" onSubmit={onSubmit}>
      ...
      <div className="mt-4 space-y-3">
        <TextField label="거래처명" required value={form.name} onChange={(value) => onChange("name", value)} />
        <div className="grid grid-cols-2 gap-3">
          <TextField label="대표자" value={form.ownerName} onChange={(value) => onChange("ownerName", value)} />
          <TextField label="전화번호" value={form.phone} onChange={(value) => onChange("phone", value)} />
        </div>
        <TextField label="주소" value={form.address} onChange={(value) => onChange("address", value)} />
