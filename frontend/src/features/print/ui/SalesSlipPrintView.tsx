    ...
    <main className="print-root mx-auto max-w-[900px] space-y-4">
      <div className="print-hidden flex flex-wrap items-center justify-between gap-3 rounded-md border border-[#d7ddd4] bg-white p-4 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-[#5c6a60]">아래 전표 영역만 인쇄됩니다.</p>
        </div>
        <div className="flex gap-2">
          <Link className="inline-flex items-center rounded-md border border-[#cfd8cc] px-4 py-2 text-sm font-semibold text-[#435047]" href="/sales">
            판매 관리
          </Link>
          <button className="rounded-md bg-[#159447] px-4 py-2 text-sm font-semibold text-white" onClick={() => window.print()} type="button">
            인쇄
          </button>
        </div>
      </div>
      ...
      <article className="a5-sheet mx-auto bg-white p-[12mm] text-[#1f2a24] shadow-sm">
        <header className="border-b-2 border-[#1f2a24] pb-4">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <h2 className="mt-1 text-3xl font-bold tracking-normal">판매 전표</h2>
            </div>
        ...
        <section className="mt-5 grid grid-cols-2 gap-3 text-sm">
          ...
          <InfoBox className="col-span-2" label="주소" value={slip.customer.address ?? "-"} />
        </section>
            ...
            <tbody>
              {slip.items.map((item) => (
                <tr key={item.id} className="border-b border-[#d8ded5]">
                  ...
                  <td className="px-2 py-2 text-right">{item.quantity.toLocaleString()}</td>
                  <td className="px-2 py-2 text-right">{item.unitPrice.toLocaleString()}</td>
                  <td className="px-2 py-2 text-right font-semibold">{item.amount.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <section className="mt-5 grid grid-cols-[1fr_190px] gap-4">
          <div className="rounded-md border border-[#cfd8cc] p-3 text-sm">
            ...
            <p className="mt-2 min-h-[46px] whitespace-pre-wrap text-[#405148]">{slip.memo ?? "-"}</p>
          </div>
          <div className="rounded-md border-2 border-[#1f2a24] p-3 text-right">
            <p className="text-sm font-bold">합계 금액</p>
            <p className="mt-3 text-2xl font-bold">{slip.totalAmount.toLocaleString()}원</p>
          </div>
        </section>
...
function InfoBox({ className = "", label, value }: { className?: string; label: string; value: string }) {
  return (
