...
export default function OrchidMovePanel({
  ...
  const [physicalBedId, setPhysicalBedId] = useState(
    currentHouse.physicalBeds[0]?.id ?? 0,
  );
  const [bedZoneId, setBedZoneId] = useState(
    currentHouse.physicalBeds[0]?.bedZones[0]?.id ?? 0,
  );
  ...
  const selectedBed =
    destinationHouse.physicalBeds.find((bed) => bed.id === physicalBedId) ??
    destinationHouse.physicalBeds[0] ??
    null;
  const safeBedZoneId = selectedBed?.bedZones.some(
    (zone) => zone.id === bedZoneId,
  )
    ? bedZoneId
  ...
  return (
    <section className="rounded-md border border-[#b9d0ff] bg-white p-3 shadow-sm">
      ...
      <p className="mt-2 text-xs text-[#5c6a60]">
        현재 위치: {selectedOrchidGroup.houseNumber}동 /{" "}
      ...
      <form className="mt-3 space-y-2" onSubmit={handleSubmit}>
        <label className="block">
          ...
          <select
            ...
            onChange={(event) =>
              setDestinationHouseId(Number(event.target.value))
            }
          >
            {houses.map((house) => (
        ...
        {loadError ? (
          <p className="rounded-md bg-[#fff1ec] p-2 text-xs text-[#9b341e]">
            {loadError}
          </p>
        ) : null}
        <button
