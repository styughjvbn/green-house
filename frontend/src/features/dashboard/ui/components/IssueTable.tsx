...
export function IssueTable({ houses }: { houses: HouseStatusSummary[] }) {
  ...
  return (
    ...
    >
        ...
        <tbody>
          {rows.map((house) => (
            <tr key={house.houseId} className="border-b border-[#edf0ec]">
              <td className="py-2 font-semibold text-[#246df2]">
                {house.houseNumber}동
              </td>
              <td className="py-2">확인 필요 {house.warningCount}건</td>
              <td className="py-2">
                <span className="rounded-md bg-[#fff1d6] px-2 py-1 text-xs font-bold text-[#d88400]">
                  주의
                </span>
              </td>
            </tr>
      ...
      {rows.length === 0 ? (
        <DashboardEmptyText text="상태 이상 또는 주의 항목이 없습니다." />
      ) : null}
    </DashboardPanel>
  );
}
