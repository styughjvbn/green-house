    ...
    <div className="rounded-xl border border-white/60 bg-white/80 p-4 shadow-sm backdrop-blur">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2 text-xs font-semibold">
          <span className="rounded-full bg-[#e7f0e6] px-3 py-1 text-[#34503b]">
            실제 방향
          </span>
          <span className="rounded-full bg-white px-3 py-1 text-[#5b6a60]">
            좌→우
          </span>
        </div>
      </div>
...
function PhysicalBedMapBlock({
  ...
  return (
    <div
      className={`rounded-lg border bg-[#f8faf7] p-3 shadow-sm ${selected ? "border-[#1976f3] ring-2 ring-[#1976f3]" : "border-[#d5ded0]"}`}
    >
      <button
        className="mb-3 flex w-full items-center justify-between rounded-md bg-[#155c30] px-3 py-2 text-left text-sm font-semibold text-white"
      ...
      <div className="grid min-h-[250px] grid-cols-2 gap-2">
        {bed.bedZones.map((zone) => (
          <BedZoneMapBlock
            ...
            showDetail={showZoneDetail}
            zone={zone}
            onSelect={() => onSelectBedZone(zone)}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <button
      className={`flex min-h-[250px] flex-col rounded-md border p-2 text-left transition hover:border-[#1976f3] ${
        selected
          ? "border-[#1976f3] bg-[#e7f0ff] ring-2 ring-[#1976f3]"
          : "border-[#cbd6c7] bg-[#edf1ea]"
      }`}
      onClick={onSelect}
      type="button"
    >
      ...
      <div className="mt-2 flex flex-1 flex-col gap-1.5">
        {zone.orchidGroups.length > 0 ? (
          zone.orchidGroups.slice(0, showDetail ? 8 : 5).map((group) => (
            <span
              key={group.id}
              className={`rounded px-2 py-1 text-xs font-semibold ${group.status === "정상" ? "bg-[#2e9d4d] text-white" : "bg-[#f59e0b] text-white"}`}
            >
              {showDetail
          ...
          <span className="flex flex-1 items-center justify-center rounded border border-dashed border-[#bec8ba] text-xs text-[#748078]">
            빈 구역
          </span>
        )}
      </div>
