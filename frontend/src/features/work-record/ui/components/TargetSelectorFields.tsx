...
export function TargetSelectorFields({
  ...
  return (
    <>
      <SelectField label="대상 유형" value={form.targetType} onChange={(value) => onChange("targetType", value as WorkRecordTargetType)}>
      ...
      {form.targetType !== "FARM" ? (
        <SelectField label="동" value={form.houseId} onChange={(value) => onChange("houseId", value)}>
          {houses.map((house) => (
      ...
      {form.targetType === "PHYSICAL_BED" ? (
        <SelectField label="배드" value={safePhysicalBedId} onChange={(value) => onChange("physicalBedId", value)}>
      ...
      {form.targetType === "BED_ZONE" ? (
        <SelectField label="구역" value={safeBedZoneId} onChange={(value) => onChange("bedZoneId", value)}>
          {bedZones.map((zone) => (
      ...
      {form.targetType === "ORCHID_GROUP" ? (
        <SelectField label="난 묶음" value={safeOrchidGroupId} onChange={(value) => onChange("orchidGroupId", value)}>
