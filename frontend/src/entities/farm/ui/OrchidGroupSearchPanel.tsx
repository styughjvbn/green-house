...
export type OrchidSearchGroupOption = {
  key: string;
    ...
    >
      ...
      {hasActiveSearch ? (
        <div className="mt-2 rounded-md border border-[#e3e8e1] bg-[#fafcf9]">
          {groupLoading || groups.length > 0 || groupError ? (
            <div className="border-b border-[#e3e8e1] p-2">
              <p className="mb-1.5 text-[11px] font-semibold text-[#6d786f]">
                관련 그룹
              </p>
              {groupLoading ? (
                ...
                <div className="max-h-[180px] space-y-1 overflow-y-auto">
                  {groups.map((group) => (
                    ...
                    >
                      ...
                      >
                        {group.type === "DERIVED" ? "자동 그룹" : "사용자 그룹"}
                      </span>
                      <span className="block truncate text-xs font-semibold text-[#26352b]">
                        {group.label}
                      </span>
                      <span className="block text-[11px] text-[#6a766e]">
                        {group.description}
                      </span>
                    </button>
                  ))}
              ...
              {groupError ? (
                <p className="mt-1 text-[11px] text-[#9b341e]">{groupError}</p>
              ) : null}
            </div>
          ) : null}
          <button
