...
class FarmStructureIntegrationTests extends FarmFixtureIntegrationTest {
	...
	void returnsFarmStructureApisWithCommonResponseShape() throws Exception {
		...
		mockMvc
			.perform(get("/api/orchid-groups").param("houseId", sampleHouse.getId().toString())
	...
	void returnsFarmStatusOrchidGroupsByTargetScope() throws Exception {
		...
		mockMvc
			.perform(get("/api/farm-status/orchid-groups").param("targetType", "HOUSE")
		...
		mockMvc
			.perform(get("/api/farm-status/orchid-groups").param("targetType", "PHYSICAL_BED")
		...
		mockMvc
			.perform(get("/api/farm-status/orchid-groups").param("targetType", "BED_ZONE")
	...
	void returnsFarmStatusZoomData() throws Exception {
		...
		mockMvc
			.perform(get("/api/farm-status/zoom").param("level", "HOUSE")
		...
		mockMvc
			.perform(get("/api/farm-status/zoom").param("level", "BED_ZONE")
				.param("physicalBedId", sampleBed.getId().toString()))
			.andExpect(status().isOk())
	...
	void returnsValidationErrorsForInvalidFarmStatusRequests() throws Exception {
		mockMvc.perform(get("/api/farm-status/orchid-groups").param("targetType", "INVALID").param("targetId", "1"))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error.code").value("VALIDATION_ERROR"));

		mockMvc.perform(get("/api/farm-status/orchid-groups").param("targetType", "HOUSE"))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error.code").value("VALIDATION_ERROR"));

		mockMvc.perform(get("/api/farm-status/orchid-groups").param("targetType", "HOUSE").param("targetId", "999999"))
			.andExpect(status().isNotFound())
			.andExpect(jsonPath("$.error.code").value("NOT_FOUND"));

		mockMvc.perform(get("/api/farm-status/zoom").param("level", "HOUSE"))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error.code").value("VALIDATION_ERROR"));
	}
