...
class VarietyAuditIntegrationTest extends AbstractBackendIntegrationTest {
	...
	void recordsVarietyCreateUpdateDeactivateAndDelete() throws Exception {
		var created = mockMvc
			.perform(post("/api/varieties").with(user("operator"))
		...
		mockMvc
			.perform(patch("/api/varieties/{id}", varietyId).with(user("operator"))
				.contentType(MediaType.APPLICATION_JSON)
				.content("""
		...
		mockMvc.perform(patch("/api/varieties/{id}/deactivate", varietyId).with(user("operator")))
			.andExpect(status().isOk());
		mockMvc.perform(delete("/api/varieties/{id}", varietyId).with(user("operator"))).andExpect(status().isOk());
		...
		assertThat(events.get(1).getChangedFields()).contains("name", "alias", "defaultPotSize", "color", "saleEnabled",
				"description", "memo");
