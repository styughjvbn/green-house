...
class SalesAnalyticsResponseAssemblerTest {
	...
	void fillsTheLastSixCalendarMonthsInOrderAcrossTheYearBoundary() {
		var values = Map.of(YearMonth.of(2025, 8), 999L, YearMonth.of(2025, 9), 100L,
				YearMonth.of(2025, 12), 300L, YearMonth.of(2026, 2), 500L, YearMonth.of(2026, 3), 999L);
	...
	void rendersPaymentGroupsInTheExistingOrderWithoutLosingLargeAmounts() {
		assertThat(SalesAnalyticsResponseAssembler.paymentBreakdown(Map.of(
				SalesPaymentCategory.UNPAID, 500L, SalesPaymentCategory.PARTIAL, 3_000_000_000L,
				SalesPaymentCategory.PAID, 2_000_000_000L))).containsExactly(
				new AnalyticsRankedValueResponse("입금 완료", 2_000_000_000L),
	...
	void keepsZeroGroupsAndRemovesTheActionWhenNoAmountIsUnpaid() {
		...
		assertThat(SalesAnalyticsResponseAssembler.salesInsights(0)).containsExactly(
				new AnalyticsInsightResponse("green", "현재 기간 미수 전표 없음", null, null));
	}
	...
	void preservesTheExistingInsightTextActionAndFormattingLocale() {
		...
		try {
			...
			assertThat(SalesAnalyticsResponseAssembler.salesInsights(3_000_000_000L)).containsExactly(
					new AnalyticsInsightResponse("red", "미수 전표 확인 필요: 3,000,000,000원", "판매 관리", "/sales"));
