...
class MovementBatchWorkOperationIntegrationTests extends AbstractBackendIntegrationTest {
	...
	void setUp() {
		...
		firstVariety = varietyRepository.save(new Variety(
				"MOVE-BATCH-1", "팔레놉시스", "이동 품종 1", null, "4치", true, true, null, null));
		secondVariety = varietyRepository.save(new Variety(
				"MOVE-BATCH-2", "덴드로비움", "이동 품종 2", null, "3치", true, true, null, null));
	}
	...
	void movesMultipleSourcesOfOneVarietyAndRecordsDiscardedQuantity() throws Exception {
		...
		mockMvc.perform(post("/api/work-operations/{id}/structure-change-executions", operationId)
				...
				.content("""
						...
						sourceZone.getId(), first.getId(), second.getId())))
				.andExpect(status().isCreated())
		...
		assertThat(results).extracting(group -> group.getVariety().getId())
		...
		assertThat(discardOperations.getFirst().getDetails())
				.containsEntry("movementOperationId", operationId);
	...
	void createsCompletedMovementRecordForMultipleSourcesOfOneVariety() throws Exception {
		...
		mockMvc.perform(post("/api/work-operations/structure-change-records/batch")
				...
				.content("""
						...
						""".formatted(
						movementType.getId(), first.getId(), second.getId(),
						first.getId(), second.getId(),
		...
		assertThat(orchidGroupRepository.findByBedZoneIdAndQuantityGreaterThanOrderBySortOrderAsc(
				destinationZone.getId(), 0)).extracting(OrchidGroup::getQuantity).containsExactly(9, 18);
	}
	...
	private OrchidGroup createSource(
		...
		OrchidGroup group = new OrchidGroup(
				sourceZone, variety.getGenus(), variety.getName(), quantity, variety.getDefaultPotSize(),
				2, "정상", sortOrder, BigDecimal.valueOf(start), BigDecimal.valueOf(end));
