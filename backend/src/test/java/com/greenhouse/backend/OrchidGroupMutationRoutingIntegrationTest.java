...
class OrchidGroupMutationRoutingIntegrationTest extends AbstractBackendIntegrationTest {
	...
	@Autowired
	private InboundPottingOperationService inboundPottingOperationService;

	@Autowired
	private WorkOperationPlanService workOperationPlanService;

	@Autowired
	private WorkOperationProgressService workOperationProgressService;

	@Autowired
	private VarietyService varietyService;

	@Autowired
	private SalesSlipCreationService salesSlipCreationService;

	@Autowired
	private SalesSlipStatusService salesSlipStatusService;
	...
	@Autowired
	private SalesInventoryMovementRepository salesInventoryMovementRepository;
	...
	void routesFarmAndImmediateInboundWritesToMutationEngine() {
		...
		orchidGroupCommandService.update(created.id(), new OrchidGroupUpdateRequest(fixture.variety().getId(), 18,
		...
		varietyService.update(fixture.variety().getId(), new VarietyUpdateRequest(fixture.variety().getGenus(),
				fixture.variety().getName(), "별칭", "4인치", null, true, null, null));
		assertThat(mutationRepository.count()).isEqualTo(mutationCountBeforeMetadataUpdate);
		varietyService.update(fixture.variety().getId(), new VarietyUpdateRequest(fixture.variety().getGenus(),
				"라우팅 변경 품종", "별칭", "4인치", null, true, null, null));
		...
		assertThat(mutationRepository.findAll()).extracting(mutation -> mutation.getSourceDomain())
			.contains(OrchidGroupMutationSourceDomain.FARM, OrchidGroupMutationSourceDomain.INBOUND);
		assertThat(workAppliedEffectRepository.findAll()).anySatisfy(effect -> {
	...
	void routesRepotWorkAndLinksEffectAndLineageToOneMutation() {
		...
		var response = repotWorkOperationService.execute(
				new RepotWorkOperationRequest("routing-repot-9962", "라우팅 분갈이", LocalDate.of(2026, 8, 20), "작업자",
						"부분 분갈이", source.id(), 10, List.of(new RepotResultOrchidGroupRequest(fixture.zone().getId(), 10,
								"4인치", 2, "단일", null, false, new BigDecimal("2"), new BigDecimal("3"), "결과")),
						Set.of()));

		entityManager.flush();
		var effect = workAppliedEffectRepository.findByWorkOperationIdOrderByIdAsc(response.operation().id())
			.getFirst();
		var lineage = lineageRepository.findBySourceOrchidGroupIdOrderByCreatedAtAscIdAsc(source.id()).getFirst();
		assertThat(effect.getMutationId()).isNotNull();
		assertThat(effect.getCorrelationId()).isNotNull();
		assertThat(lineage.getMutationId()).isEqualTo(effect.getMutationId());
		assertThat(orchidGroupRepository.findById(source.id()).orElseThrow().getStateRevision()).isEqualTo(2L);
		assertThat(orchidGroupRepository.findById(response.resultOrchidGroups().getFirst().id())
			.orElseThrow()
			.getStateRevision()).isEqualTo(1L);
	}
	...
	void routesSalesReservationOutboundAndCompensationWithMovementLinks() {
		...
		BusinessPartner partner = businessPartnerRepository
			.save(new BusinessPartner("라우팅 판매처", PartnerType.WHOLESALE, null, null, null, null));
		var slip = salesSlipCreationService.create(new SalesSlipCommand(LocalDate.of(2026, 8, 20), SalesType.DIRECT,
				partner.getId(), null, "미입금", SalesSlip.STATUS_DRAFT, null, "엔진 판매",
				List.of(new SalesSlipItemInput(fixture.variety().getName(), fixture.variety().getGenus(), "4인치", 3,
						10_000, null, List.of(new SalesSlipAllocationInput(group.id(), 3))))));
		salesSlipUpdateService.update(slip.id(),
				new SalesSlipCommand(LocalDate.of(2026, 8, 20), SalesType.DIRECT, partner.getId(), null, "미입금",
						SalesSlip.STATUS_DRAFT, null, "엔진 판매 수정",
						List.of(new SalesSlipItemInput(fixture.variety().getName(), fixture.variety().getGenus(), "4인치",
								4, 10_000, null, List.of(new SalesSlipAllocationInput(group.id(), 4))))));

		salesSlipStatusService.updateStatus(slip.id(),
				new SalesSlipStatusUpdateRequest(SalesSlip.STATUS_DIRECT_OUTBOUND_COMPLETED, null));
		salesSlipStatusService.updateStatus(slip.id(),
				new SalesSlipStatusUpdateRequest(SalesSlip.STATUS_CANCELED, null));
		entityManager.flush();
	...
	void routesInboundPottingResultsThroughWorkMutation() {
		...
		Long groupId = effect.getResultDetails().get("createdOrchidGroupIds") instanceof List<?> ids
				? ((Number) ids.getFirst()).longValue() : null;
		assertThat(groupId).isNotNull();
		assertThat(orchidGroupRepository.findById(groupId).orElseThrow().getStateRevision()).isEqualTo(1L);
		assertThat(effect.getMutationId()).isNotNull();
		assertThat(effect.getCorrelationId()).isNotNull();
	...
	private Fixture createFixture(int houseNumber, String suffix) {
		...
		Variety variety = varietyRepository
			.save(new Variety("ROUTING-" + houseNumber, "팔레놉시스", suffix, null, "4인치", true, true, null, null));
		return new Fixture(zone, variety);
	}

	private OrchidGroupCreateRequest groupRequest(Fixture fixture, int quantity, String start, String end,
			String status) {
		return new OrchidGroupCreateRequest(fixture.zone().getId(), fixture.variety().getId(), quantity, "4인치", 2,
				status, "단일", null, false, new BigDecimal(start), new BigDecimal(end), null);
	}
