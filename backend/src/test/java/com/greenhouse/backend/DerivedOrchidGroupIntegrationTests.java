...
class DerivedOrchidGroupIntegrationTests extends AbstractBackendIntegrationTest {
	...
	void recalculatesGroupsImmediatelyFromCurrentOrchidAttributes() throws Exception {
		...
		Variety variety = varietyRepository
			.save(new Variety("DERIVED-TEST", "팔레놉시스", "자동 그룹 테스트 난", null, "3.5\"", true, true, null, null));
		...
		mockMvc
			.perform(get("/api/orchid-groups/derived-groups").param("varietyId", variety.getId().toString())
	...
	private OrchidGroup saveGroup(BedZone zone, Variety variety, int quantity, String potSize, int ageYear,
