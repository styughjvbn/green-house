...
class SalesIntegrationTests extends FarmFixtureIntegrationTest {
	...
	void createsBusinessPartnersAndSalesSlipsWithCalculatedAmounts() throws Exception {
		...
		mockMvc
			.perform(put("/api/business-partners/{partnerId}", partnerId).contentType(MediaType.APPLICATION_JSON)
				.content("""
						{
						  "name": "??? ??",
						  "partnerType": "RETAIL",
		...
		var slipResult = mockMvc
			.perform(post("/api/sales-slips").contentType(MediaType.APPLICATION_JSON)
				.content("""
						...
						""".formatted(partnerId, firstGroup.getVarietyName(), firstGroup.getGenus(), firstGroup.getId(),
						secondGroup.getVarietyName(), secondGroup.getGenus(), secondGroup.getId())))
			.andExpect(status().isCreated())
		...
		mockMvc
			.perform(get("/api/sales-slips").param("partnerId", partnerId.toString())
	...
	void returnsValidationErrorsForInvalidSalesRequests() throws Exception {
		...
		mockMvc
			.perform(post("/api/business-partners").contentType(MediaType.APPLICATION_JSON)
	...
	void updatesDraftDirectSalesSlipAndRecalculatesReservations() throws Exception {
		...
		mockMvc
			.perform(put("/api/sales-slips/{salesSlipId}", salesSlipId).contentType(MediaType.APPLICATION_JSON)
				.content("""
						{
						  "saleDate": "2026-07-09",
						  "partnerId": %d,
						...
						""".formatted(partnerId, firstGroup.getVarietyName(), firstGroup.getGenus(), firstGroup.getId(),
						thirdGroup.getVarietyName(), thirdGroup.getGenus(), thirdGroup.getId())))
			.andExpect(status().isOk())
