...
class BusinessPartnerAuditIntegrationTest extends AbstractBackendIntegrationTest {
	...
	void recordsCreateAndUpdateWithoutPersonalInformation() throws Exception {
		var created = mockMvc
			.perform(post("/api/business-partners").with(user("partner-operator"))
				.header("X-Request-Id", "partner-create")
				.contentType(MediaType.APPLICATION_JSON)
				.content(requestJson("감사 도매처", "WHOLESALE", "김대표", "010-1234-5678", "개인 주소", "민감 메모")))
			.andExpect(status().isCreated())
			.andReturn();
		Long partnerId = Long
			.valueOf(created.getResponse().getContentAsString().replaceAll(".*\\\"id\\\":(\\d+).*", "$1"));

		mockMvc
			.perform(put("/api/business-partners/{partnerId}", partnerId).with(user("partner-operator"))
				.header("X-Request-Id", "partner-update")
				.contentType(MediaType.APPLICATION_JSON)
				.content(requestJson("감사 소매처", "RETAIL", "박대표", "010-9999-8888", "변경 주소", "변경 메모")))
		...
		assertThat(events.getLast().getChangedFields()).containsExactly("name", "partnerType", "ownerName", "phone",
				"address", "memo");
		assertThat(events).allSatisfy(event -> {
			assertThat(event.getEntityType()).isEqualTo("BUSINESS_PARTNER");
			assertThat(event.getActorId()).isEqualTo("partner-operator");
			assertThat(event.getBeforeData().toString()).doesNotContain("김대표", "010-1234-5678", "개인 주소", "민감 메모", "박대표",
					"010-9999-8888", "변경 주소", "변경 메모");
			assertThat(event.getAfterData().toString()).doesNotContain("김대표", "010-1234-5678", "개인 주소", "민감 메모", "박대표",
					"010-9999-8888", "변경 주소", "변경 메모");
		});
	}
	...
	void skipsAuditForUnchangedUpdate() throws Exception {
		...
		assertThat(auditEventRepository.findAll()
			.stream()
			.filter(event -> event.getSource() == AuditSource.PARTNER_MANAGEMENT)
			.filter(event -> event.getEntityId().equals(partnerId))).hasSize(1);
	}

	private String requestJson(String name, String partnerType, String ownerName, String phone, String address,
			String memo) {
		return """
				{
				  "name": "%s",
