...
@ActiveProfiles("test")
@Transactional
class AuctionTrackingTests extends FarmFixtureIntegrationTest {

	@Autowired
	AuctionTrackingService trackingService;

	@Autowired
	AuctionShipmentLotRepository lotRepository;
	...
	void confirmsReturnAndAdjustsQuantitiesWithHistory() {
		...
		var adjusted = trackingService.adjust(lot.getId(), new AuctionLotAdjustmentRequest(10, 0, 40, "관리자", "수량 보정"));
	...
	void createsAuctionShipmentAndLotsWhenAuctionSalesSlipIsCreatedAsCompleted() throws Exception {
		...
		var sampleGroups = orchidGroupRepository.findAll()
			.stream()
		...
		String request = """
				...
				""".formatted(auctionHouse.getId(), sampleGroups.get(0).getVarietyName(),
				sampleGroups.get(0).getGenus(), sampleGroups.get(0).getId(), sampleGroups.get(1).getVarietyName(),
				sampleGroups.get(1).getGenus(), sampleGroups.get(1).getId());

		mockMvc.perform(post("/api/sales-slips").contentType(MediaType.APPLICATION_JSON).content(request))
	...
	void createsDraftAuctionSalesSlipWithoutShipmentAndMaterializesOnCompletion() throws Exception {
		...
		var sampleGroup = orchidGroupRepository.findAll()
			.stream()
			.filter(group -> group.getVariety() != null)
			.findFirst()
			.orElseThrow();
		...
		var createResult = mockMvc
			.perform(post("/api/sales-slips").contentType(MediaType.APPLICATION_JSON)
				.content("""
						{
						  "saleDate": "2026-07-08",
						  "salesType": "AUCTION",
						...
						""".formatted(auctionHouse.getId(), sampleGroup.getVarietyName(), sampleGroup.getGenus(),
						sampleGroup.getId())))
			.andExpect(status().isCreated())
		...
		mockMvc
			.perform(patch("/api/sales-slips/{salesSlipId}/sales-status", salesSlipId)
	...
	void rejectsAuctionSalesSlipCancelWhenAuctionResultAlreadyExists() throws Exception {
		...
		var createResult = mockMvc
			.perform(
					post("/api/sales-slips").contentType(MediaType.APPLICATION_JSON)
						.content("""
								{
								  "saleDate": "2026-07-08",
								  "salesType": "AUCTION",
								...
								""".formatted(auctionHouse.getId(), sampleGroup.getVarietyName(),
								sampleGroup.getGenus(), sampleGroup.getId())))
			.andExpect(status().isCreated())
			.andReturn();
		var salesSlipId = Long
			.valueOf(createResult.getResponse().getContentAsString().replaceFirst(".*?\\\"id\\\":(\\d+).*", "$1"));
		...
		trackingService.addResult(createdLot.getId(), new RecordAuctionResultCommand(LocalDate.of(2026, 7, 9), 1,
				AuctionAttemptStatus.FAILED, "유찰", null, null));
	...
	void recordsManualAuctionResults() throws Exception {
		...
		assertThat(partialResponse.attempts().getFirst().attemptStatus())
			.isEqualTo(AuctionAttemptStatus.PARTIALLY_SOLD);
		assertThat(partialResponse.attempts().getFirst().resultLines()).hasSize(2);

		mockMvc
			.perform(post("/api/auction-lots/" + returnLot.getId() + "/results").contentType(MediaType.APPLICATION_JSON)
				.content("""
						{
	...
	private AuctionShipmentLot createLot(LocalDate shipmentDate, String market, String variety, String grade,
			int quantity) {
	...
	private void addResult(AuctionShipmentLot lot, LocalDate auctionDate, int attemptNo, int quantity, int unitPrice,
			String note, AuctionAttemptStatus status) {
		var attempt = new AuctionAttempt(auctionDate, attemptNo, status, note, null);
		attempt.addResultLine(new AuctionResultLine(auctionDate, lot.getShipmentGrade(), quantity, unitPrice,
				quantity * unitPrice, note, AuctionInspectionStatus.NORMAL));
