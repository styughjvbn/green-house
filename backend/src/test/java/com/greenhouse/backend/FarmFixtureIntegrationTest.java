...
abstract class FarmFixtureIntegrationTest extends AbstractBackendIntegrationTest {
	...
	void createFarmFixture() {
		...
		for (int index = 0; index < 3; index++) {
			var variety = new Variety("V-FIX-" + index, "카틀레야", "카틀레야 " + (char) ('A' + index),
					null, "4치", true, true, null, null);
			fixtureEntityManager.persist(variety);
			var group = new OrchidGroup(occupiedZone, variety.getGenus(), variety.getName(), 300, "4치", 1,
					"정상", index + 1, BigDecimal.valueOf(index * 7), BigDecimal.valueOf((index + 1) * 7));
			group.assignVariety(variety);
			fixtureEntityManager.persist(group);
			fixtureEntityManager.persist(new Material("M-FIX-" + index, "자재", "자재 " + index,
					null, null, null, null, null, true));
		}
		for (var template : WorkTypeTemplate.values()) {
			if (workTypeRepository.findByCode(template.name()).isEmpty()) {
				fixtureEntityManager.persist(new WorkType(template.name(), template == WorkTypeTemplate.MOVEMENT ? "자리 이동" : template.name(), template,
						true, false, true, template.ordinal()));
			}
		}
