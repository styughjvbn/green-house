...
class WorkOperationIntegrationTests extends AbstractBackendIntegrationTest {
	...
	void setUp() {
		...
		movementType = workTypeRepository
			.save(new WorkType("MOVEMENT", "자리 이동", WorkTypeTemplate.MOVEMENT, true, true, true, 2));
		discardType = workTypeRepository
			.save(new WorkType("DISCARD", "폐기", WorkTypeTemplate.DISCARD, true, true, true, 3));
		repotType = workTypeRepository
			.save(new WorkType(WorkTypeDefinition.REPOT.name(), "분갈이", WorkTypeTemplate.REPOT, true, true, true, 4));
	...
	void exposesOperationAndTargetActionsFromTheBackendContract() throws Exception {
		...
		mockMvc.perform(post("/api/work-operations/{id}/start", operationId))
			...
			.andExpect(jsonPath("$.data.targets[0].availableActions", containsInAnyOrder("START", "COMPLETE", "SKIP")));
	}
	...
	void excludesLegacyMigrationMetadataFromOperationDetails() throws Exception {
		mockMvc.perform(post("/api/work-operations").contentType(MediaType.APPLICATION_JSON).content("""
				...
				""".formatted(pesticideType.getId(), targetGroup.getId()))).andExpect(status().isCreated());
	...
	void batchCreateSplitsSingleVarietyStructureWorkInOneRequest() throws Exception {
		Variety anotherVariety = varietyRepository
			.save(new Variety("TEST-002", "카틀레야", "다른 품종", null, "3.5치", true, true, null, null));
	...
	void discardsPartialOrFullQuantityAndClosesFullyDiscardedGroup() throws Exception {
		...
		mockMvc.perform(post("/api/work-operations/{id}/start", partialOperationId)).andExpect(status().isOk());
		mockMvc
		...
		mockMvc.perform(post("/api/work-operations/{id}/complete", partialOperationId)).andExpect(status().isOk());
		...
		Long fullTargetId = workOperationTargetRepository
			...
			.getFirst()
			.getId();
		mockMvc.perform(post("/api/work-operations/{id}/start", fullOperationId)).andExpect(status().isOk());
		mockMvc
			.perform(post("/api/work-operations/{id}/targets/{targetId}/complete", fullOperationId, fullTargetId)
				.contentType(MediaType.APPLICATION_JSON)
				.content("""
						{
	...
	void plansAndExecutesMovementPerTargetWithoutLegacyRecord() throws Exception {
		var createResult = mockMvc
			.perform(post("/api/work-operations").contentType(MediaType.APPLICATION_JSON).content("""
					{
					  "workTypeId": %d,
					  "title": "5동 자리 이동",
					  "plannedStartDate": "2026-07-16",
	...
	void preservesHouseTargetSnapshotAfterOrchidGroupMoves() throws Exception {
		mockMvc.perform(post("/api/work-operations/target-preview").contentType(MediaType.APPLICATION_JSON).content("""
				{
		...
		Long operationId = Long.valueOf(response.replaceAll(".*?\\\"data\\\":\\{\\\"id\\\":(\\d+).*", "$1"));
		Long targetId = workOperationTargetRepository.findByWorkOperationIdAndExcludedAtIsNullOrderByIdAsc(operationId)
			.getFirst()
			.getId();
		...
		mockMvc
			.perform(post("/api/work-operations/{id}/complete", operationId).contentType(MediaType.APPLICATION_JSON)
		...
		mockMvc
			.perform(get("/api/work-history").param("historyScopeType", "HOUSE")
		...
		mockMvc.perform(get("/api/orchid-groups/{id}/work-history", targetGroup.getId()))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data", hasSize(2)))
			.andExpect(jsonPath("$.data[?(@.sourceKind == 'WORK_OPERATION')].workOperationId")
				.value(hasItem(operationId.intValue())))
			.andExpect(jsonPath("$.data[?(@.sourceKind == 'WORK_OPERATION')].propagated").value(hasItem(true)))
			.andExpect(jsonPath("$.data[?(@.sourceKind == 'WORK_OPERATION')].locationSnapshot.houseNumber")
				.value(hasItem(3)))
			.andExpect(jsonPath("$.data[?(@.sourceKind == 'WORK_OPERATION')].currentLocation.houseNumber")
				.value(hasItem(5)))
			.andExpect(jsonPath("$.data[?(@.sourceKind == 'WORK_OPERATION')].workType").value(hasItem("자리 이동")));

		mockMvc
			.perform(get("/api/work-history").param("historyScopeType", "ORCHID_GROUP")
	...
	void searchesOperationsByOverlappingPeriodStatusAndScope() throws Exception {
		mockMvc.perform(post("/api/work-operations").contentType(MediaType.APPLICATION_JSON).content("""
				...
				""".formatted(pesticideType.getId(), sourceHouse.getId()))).andExpect(status().isCreated());
		mockMvc.perform(post("/api/work-operations").contentType(MediaType.APPLICATION_JSON).content("""
				{
				  "workTypeId": %d,
				  "title": "7월 기간 농약 작업 2",
		...
		mockMvc
			.perform(get("/api/work-operations").param("from", "2026-07-15")
				.param("to", "2026-07-31")
		...
		mockMvc.perform(get("/api/work-operations").param("from", "2026-08-01").param("to", "2026-08-31"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.content", hasSize(0)));

		mockMvc.perform(get("/api/work-operations").param("keyword", "농약").param("page", "0").param("size", "1"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.content", hasSize(1)))
			.andExpect(jsonPath("$.data.page").value(0))
			.andExpect(jsonPath("$.data.size").value(1))
		...
		mockMvc.perform(get("/api/work-operations").param("keyword", "농약").param("page", "1").param("size", "1"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.content", hasSize(1)))
			.andExpect(jsonPath("$.data.page").value(1))
		...
		mockMvc
			.perform(get("/api/work-operations/calendar").param("from", "2026-07-01")
				.param("to", "2026-07-31")
				.param("status", "PLANNED"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data", hasSize(2)))
		...
		mockMvc.perform(get("/api/work-operations").param("size", "101")).andExpect(status().isBadRequest());
	}
	...
	void includesTodayChangedOperationInManagementList() throws Exception {
		var created = mockMvc.perform(post("/api/work-operations").contentType(MediaType.APPLICATION_JSON).content("""
				...
				""".formatted(pesticideType.getId(), sourceHouse.getId()))).andExpect(status().isCreated()).andReturn();
	...
	void cancelsRemainingTargetsAfterSomeTargetsAreCompleted() throws Exception {
		...
		mockMvc.perform(post("/api/work-operations/{id}/start", operationId)).andExpect(status().isOk());
		mockMvc
			.perform(post("/api/work-operations/{id}/targets/{targetId}/complete", operationId, completedTargetId)
				.contentType(MediaType.APPLICATION_JSON)
				.content("""
	...
	void createsAnImmediatelyCompletedRecordOperation() throws Exception {
		mockMvc.perform(post("/api/work-operations/record").contentType(MediaType.APPLICATION_JSON).content("""
				{
				  "workTypeId": %d,
				  "title": "구역 농약 기록",
