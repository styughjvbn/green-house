...
class OrchidGroupAuditSupportTest {
	...
	void detectsMultipleChanges() {
		...
		var after = new OrchidGroupAuditSnapshot(9L, 3, "5인치", 20, 1L, 2L, 3L, BigDecimal.ONE, BigDecimal.TWO, "주의");
		assertThat(support.detectChanges(before, after)).containsExactly("varietyId", "ageYear", "potSize", "quantity",
				"status");
	}
	...
	void keepsOrdinaryAndInactiveToInactiveCorrectionsAsUpdates() {
		...
		assertThat(support.actionForCorrection(active, withStatus(active, "주의"))).isEqualTo(AuditAction.UPDATED);
		var inactive = withStatus(active, "종료");
		assertThat(support.actionForCorrection(inactive, withStatus(inactive, "폐기"))).isEqualTo(AuditAction.UPDATED);
	}

	private OrchidGroupAuditSnapshot snapshot(Long varietyId, int quantity, Long houseId, Long bedId, Long zoneId) {
		return new OrchidGroupAuditSnapshot(varietyId, 2, "4인치", quantity, houseId, bedId, zoneId, BigDecimal.ONE,
				BigDecimal.TWO, "정상");
	}
