...
class MutationFingerprintCompatibilityTest {
	...
	void keepsAllPersistedCommandFingerprints() throws Exception {
		var source = new OrchidGroupMutationSource(OrchidGroupMutationSourceDomain.WORK,
				"TEST", "12", "EXECUTE:12", UUID.fromString("00000000-0000-0000-0000-000000000001"));
		var date = LocalDate.of(2026, 9, 8);
		var details = new OrchidGroupMutationDetails(3L, 10, "4치", 1, " 정상 ", "TRAY", 1, false,
				new BigDecimal("0.00"), new BigDecimal("2.00"), " memo ");
		var groups = List.of(new CreateOrchidGroupMutationItem(2L, details));
		var quantities = List.of(new OrchidGroupQuantityMutationItem(7L, 2), new OrchidGroupQuantityMutationItem(5L, 1));
		List<OrchidGroupMutationCommand> commands = List.of(
				...
				new TransformOrchidGroupsMutationCommand(source, List.of(new TransformOrchidGroupMutationSource(5L, 10, null, null)),
				...
				new MoveOrchidGroupMutationCommand(source, 5L, 2L, BigDecimal.ZERO, new BigDecimal("2.00"), date, " reason "),
				...
				new RestoreOutboundOrchidGroupsMutationCommand(source, quantities, RelatedOrchidGroupMutations.current(List.of(11L, 9L)), date, " reason "),
				new CorrectOrchidGroupsMutationCommand(source, List.of(new CorrectOrchidGroupMutationItem(5L, 8, " 정상 ")),
		...
		try (var input = getClass().getResourceAsStream("/farm/mutation-fingerprints.json")) {
			assertThat((com.fasterxml.jackson.databind.JsonNode) mapper.valueToTree(hashes)).isEqualTo(mapper.readTree(input));
		}
		assertThat(commands.stream().map(Object::getClass).collect(java.util.stream.Collectors.toSet()))
				.containsExactlyInAnyOrderElementsOf(java.util.Arrays.asList(OrchidGroupMutationCommand.class.getPermittedSubclasses()));
	}
}
