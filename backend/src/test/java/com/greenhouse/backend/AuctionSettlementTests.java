...
class AuctionSettlementTests {
	...
	void rebuildsAuctionDateSettlementFromSoldResultLines() throws Exception {
		var auctionHouse = partnerRepository
			.saveAndFlush(new BusinessPartner("정산 경매장", PartnerType.AUCTION_HOUSE, null, null, null, null));
		...
		mockMvc
			.perform(put("/api/business-partners/{partnerId}/settlement-settings", auctionHouse.getId())
		...
		var settingsAudit = auditEventRepository.findAll()
			.stream()
		...
		assertThat(settingsAudit.getChangedFields()).contains("paymentDelayDays", "paymentDayMode", "autoMatchEnabled",
		...
		mockMvc
			.perform(get("/api/auction-settlements").param("auctionHouseId", auctionHouse.getId().toString())
		...
		mockMvc
			.perform(post("/api/auction-settlements/rebuild").param("auctionHouseId", auctionHouse.getId().toString())
	...
	void pagesSummariesAndAggregatesTheSameFiltersAcrossAllPages() throws Exception {
		var house = partnerRepository
			.saveAndFlush(new BusinessPartner("목록 경매장", PartnerType.AUCTION_HOUSE, null, null, null, null));
		var otherHouse = partnerRepository
			.saveAndFlush(new BusinessPartner("다른 경매장", PartnerType.AUCTION_HOUSE, null, null, null, null));
		...
		mockMvc
			.perform(get("/api/auction-settlements/page").param("auctionHouseId", house.getId().toString())
				.param("from", firstDate.toString())
				.param("to", firstDate.plusDays(1).toString()))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.totalElements").value(2))
			.andExpect(jsonPath("$.data.content[0].id").value(second.getId()))
			.andExpect(jsonPath("$.data.content[1].id").value(first.getId()));
		mockMvc
			.perform(get("/api/auction-settlements/summary").param("auctionHouseId", house.getId().toString())
				.param("from", firstDate.toString())
				.param("to", firstDate.plusDays(1).toString()))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.expectedDepositAmount").value(2_000_000_000L))
			.andExpect(jsonPath("$.data.remainingAmount").value(900_000_000L));

		for (var path : new String[] { "/page", "/summary" }) {
			var result = mockMvc
				.perform(get("/api/auction-settlements" + path).param("auctionHouseId", house.getId().toString())
					.param("from", firstDate.toString())
					.param("to", firstDate.plusDays(1).toString())
	...
	void emptySummaryIsZeroAndLegacyLimitDoesNotTruncateGlobalTotals() throws Exception {
		mockMvc.perform(get("/api/auction-settlements/summary").param("auctionHouseId", "-1"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.expectedDepositAmount").value(0))
			.andExpect(jsonPath("$.data.remainingAmount").value(0));
		var house = partnerRepository
			.saveAndFlush(new BusinessPartner("누적 경매장", PartnerType.AUCTION_HOUSE, null, null, null, null));
		...
		mockMvc.perform(get("/api/auction-settlements/summary").param("auctionHouseId", house.getId().toString()))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.data.expectedDepositAmount").value(1_000))
	...
	private void createResult(BusinessPartner auctionHouse, LocalDate shipmentDate, LocalDate auctionDate,
			String variety, int quantity, int unitPrice) {
		...
		var attempt = new AuctionAttempt(auctionDate, 1,
				unitPrice > 0 ? AuctionAttemptStatus.SOLD : AuctionAttemptStatus.FAILED, unitPrice > 0 ? null : "유찰",
				null);
		attempt.addResultLine(new AuctionResultLine(auctionDate, "A", quantity, unitPrice, quantity * unitPrice,
				unitPrice > 0 ? null : "유찰", AuctionInspectionStatus.NORMAL));
