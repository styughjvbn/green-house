...
class BusinessPartnerIntegrationTests extends AbstractBackendIntegrationTest {
	...
	void filtersOptionsBeforePagingAndKeepsStableOrderForDuplicateNames() throws Exception {
		...
		mockMvc
			.perform(get("/api/business-partners/options").param("keyword", " 선택 검증 ")
				.param("active", "true")
				.param("auctionHouse", "false")
				.param("size", "1"))
	...
	void limitsLegacyListButKeepsEveryOptionReachable() throws Exception {
		...
		mockMvc
			.perform(get("/api/business-partners/options").param("keyword", "누적 거래처")
				.param("page", "50")
				.param("size", "10"))
			.andExpect(status().isOk())
	...
	void pagesAndFiltersBusinessPartners() throws Exception {
		...
		mockMvc
			.perform(get("/api/business-partners/page").param("keyword", "페이지검증")
				.param("active", "true")
				.param("page", "0")
				.param("size", "2"))
			.andExpect(status().isOk())
	...
	private void createPartner(String name, String partnerType, String memo) throws Exception {
		mockMvc.perform(post("/api/business-partners").contentType(MediaType.APPLICATION_JSON).content("""
				{
				  "name": "%s",
				  "partnerType": "%s",
				  "ownerName": null,
				...
				""".formatted(name, partnerType, memo))).andExpect(status().isCreated());
	}
