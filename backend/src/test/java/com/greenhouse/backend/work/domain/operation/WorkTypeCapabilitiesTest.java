...
class WorkTypeCapabilitiesTest {
	private static final Map<WorkTypeTemplate, String> TEMPLATE_HANDLERS = Map.of(
			WorkTypeTemplate.PESTICIDE, "RECORD_ONLY", WorkTypeTemplate.FERTILIZER, "RECORD_ONLY",
			WorkTypeTemplate.CLEANUP, "RECORD_ONLY", WorkTypeTemplate.STATUS, "RECORD_ONLY",
			WorkTypeTemplate.MEMO, "RECORD_ONLY", WorkTypeTemplate.REPOT, "REPOT",
			WorkTypeTemplate.MOVEMENT, "MOVE", WorkTypeTemplate.DISCARD, "DISCARD",
			WorkTypeTemplate.MULTI_CREATE, "MULTI_CREATE", WorkTypeTemplate.CORRECTION, "CORRECTION");
	private static final Map<WorkTypeTemplate, WorkEffectKind> TEMPLATE_KINDS = Map.of(
			WorkTypeTemplate.PESTICIDE, WorkEffectKind.RECORD_ONLY,
			WorkTypeTemplate.FERTILIZER, WorkEffectKind.RECORD_ONLY,
			WorkTypeTemplate.CLEANUP, WorkEffectKind.RECORD_ONLY,
			WorkTypeTemplate.STATUS, WorkEffectKind.RECORD_ONLY,
			WorkTypeTemplate.MEMO, WorkEffectKind.RECORD_ONLY,
			WorkTypeTemplate.CORRECTION, WorkEffectKind.RECORD_ONLY,
			WorkTypeTemplate.REPOT, WorkEffectKind.STRUCTURE_CHANGE,
			WorkTypeTemplate.MULTI_CREATE, WorkEffectKind.STRUCTURE_CHANGE,
			WorkTypeTemplate.DISCARD, WorkEffectKind.ATTRIBUTE_CHANGE,
	...
	void preservesExistingCodeTemplateAndFlagCombinations(
			String code, WorkTypeWorkflow workflow, WorkTargetReferenceType targetSource,
			boolean managedRegistration, boolean dedicatedPeriodPlan, boolean dedicatedRegistration,
			boolean structureExecution, String handlerOverride) {
		for (WorkTypeTemplate template : WorkTypeTemplate.values()) {
			for (boolean active : List.of(false, true)) {
				for (boolean system : List.of(false, true)) {
					...
					assertThat(type.handlerCode()).isEqualTo(handlerOverride == null ? TEMPLATE_HANDLERS.get(template) : handlerOverride);
	...
	void metadataStillOffersOnlyTheFiveRecordTemplates() {
		assertThat(java.util.Arrays.stream(WorkTypeTemplate.values()).filter(WorkTypeTemplate::isCustomTypeAllowed))
				.containsExactly(WorkTypeTemplate.PESTICIDE, WorkTypeTemplate.FERTILIZER,
						WorkTypeTemplate.CLEANUP, WorkTypeTemplate.STATUS, WorkTypeTemplate.MEMO);
	}
