...
class WorkEffectStoreTest {
	...
	private final WorkAppliedEffectRepository appliedEffectRepository = mock(WorkAppliedEffectRepository.class);
	...
	void persistsAndReplaysTheMutationLinkWithoutFarmDomainDependency() {
		...
		WorkExecutionResult executionResult = new WorkExecutionResult("REPOT", Map.of("result", "ok"), List.of(201L),
				mutationLink);
		WorkEffectCommand command = new WorkEffectCommand(LocalDateTime.of(2026, 8, 20, 9, 0), "worker",
				Map.of("command", "value"), null);
		when(appliedEffectRepository.save(any(WorkAppliedEffect.class)))
			.thenAnswer(invocation -> invocation.getArgument(0));

		store.save(operation, null, command, "EXECUTION:round-1", List.of(101L), WorkEffectKind.STRUCTURE_CHANGE,
				executionResult);
