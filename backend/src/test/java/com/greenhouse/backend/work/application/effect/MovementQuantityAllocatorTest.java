...
class MovementQuantityAllocatorTest {
	...
	void poolsThreeSourcesBeforeSplittingTwoResults() {
		var request = new StructureChangeCommand("pooled-movement", LocalDate.of(2026, 8, 16), null, null,
				List.of(new StructureChangeSourceInput(1L, 100, null, null),
		...
		assertThat(MovementQuantityAllocator.allocateMovedBySource(request))
			.containsExactly(java.util.Map.entry(1L, 100), java.util.Map.entry(2L, 100), java.util.Map.entry(3L, 100));
	}

	private StructureChangeResultInput result(Long bedZoneId, int quantity, String startPosition, String endPosition) {
		return new StructureChangeResultInput(bedZoneId, quantity, 1L, null, null, StructureChangeResultPurpose.NORMAL,
				null, null, false, new BigDecimal(startPosition), new BigDecimal(endPosition), null);
	}
