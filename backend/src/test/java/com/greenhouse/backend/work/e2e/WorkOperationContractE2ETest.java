...
class WorkOperationContractE2ETest extends WorkE2ETestBase {
	...
	void createsAndCompletesNormalWorkWithoutChangingItsApiContract() throws Exception {
		...
		ApiResult completed = post("/api/work-operations/%d/targets/%d/complete".formatted(operationId, targetId), """
				{
				  "worker": "E2E 완료자",
		...
		assertThat(completed.data().path("targets").get(0).path("executionStatus").asText()).isEqualTo("COMPLETED");
		...
		assertThat(detail.data().path("targets").get(0).path("executionStatus").asText()).isEqualTo("COMPLETED");
		assertThat(detail.data().path("targets").get(0).path("resultDetails").path("weather").asText()).isEqualTo("맑음");
	...
	void previewsTargetsAndCreatesAnImmediatelyCompletedRecord() throws Exception {
		...
		assertThat(workTypes.data()).anyMatch(type -> type.path("id").asLong() == scenario.pesticideWorkTypeId()
		...
		assertThat(recorded.data().path("targets").get(0).path("executionStatus").asText()).isEqualTo("COMPLETED");
	...
	void transitionsOperationAndTargetStatuses() throws Exception {
		...
		assertThat(post("/api/work-operations/%d/start".formatted(operationId), "").data().path("status").asText())
			.isEqualTo("IN_PROGRESS");
		assertThat(post("/api/work-operations/%d/pause".formatted(operationId), "").data().path("status").asText())
			.isEqualTo("PAUSED");
		...
		assertThat(post("/api/work-operations/%d/start".formatted(targetOperationId), "").status()).isEqualTo(200);
		...
		ApiResult skipped = post("/api/work-operations/%d/targets/%d/skip".formatted(targetOperationId, targetId),
				"{\"worker\":\"E2E 대상 작업자\",\"resultDetails\":{\"reason\":\"제외\"}}");
		assertThat(skipped.status()).isEqualTo(200);
		assertThat(skipped.data().path("targets").get(0).path("executionStatus").asText()).isEqualTo("SKIPPED");
