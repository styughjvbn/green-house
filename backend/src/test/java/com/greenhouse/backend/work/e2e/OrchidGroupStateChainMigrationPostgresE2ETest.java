...
class OrchidGroupStateChainMigrationPostgresE2ETest extends WorkE2ETestBase {
	...
	void importsCompleteChainsReplaysIdempotentlyAndActivatesCoverage() {
		...
		OrchidGroupStateSnapshot current = OrchidGroupStateSnapshot
			.from(orchidGroupRepository.findDetailById(scenario.orchidGroupId()).orElseThrow());
		OrchidGroupStateSnapshot observed = withQuantity(current, 88);
		OrchidGroupStateSnapshot deleted = withQuantity(current, 12);
		OrchidGroupStateChainMigrationManifest manifest = manifest(scenario.orchidGroupId(), observed, current,
				deleted);

		var validated = migrationService.validate(cutoverKey, BUSINESS_DATE, WRITER_VERSION, MANIFEST_FINGERPRINT,
				manifest);

		assertThat(validated.applied()).isFalse();
		assertThat(validated.reconciliation().stage()).isEqualTo(OrchidGroupLedgerReconciliationStage.PRE_BASELINE);
		assertThat(validated.reconciliation().ready()).isTrue();

		var imported = migrationService.importManifest(cutoverKey, BUSINESS_DATE, WRITER_VERSION, MANIFEST_FINGERPRINT,
				manifest);
		...
		assertThat(jdbcTemplate.queryForObject("SELECT state_revision FROM orchid_groups WHERE id = ?", Long.class,
			...
			.isEqualTo(1L);
		assertThat(jdbcTemplate.queryForObject(
				"SELECT COUNT(*) FROM orchid_group_mutation_entries WHERE orchid_group_id = ?", Long.class,
				DELETED_GROUP_ID))
			.isEqualTo(2L);

		var replayed = migrationService.importManifest(cutoverKey, BUSINESS_DATE, WRITER_VERSION, MANIFEST_FINGERPRINT,
				manifest);
		...
		var cutover = cutoverService.execute(
				new OrchidGroupLedgerCutoverCommand(cutoverKey, BUSINESS_DATE, WRITER_VERSION, WRITER_VERSION, true));

		assertThat(cutover.activated()).isTrue();
		assertThat(cutover.reconciliation().stage()).isEqualTo(OrchidGroupLedgerReconciliationStage.ACTIVE);
		assertThat(cutover.reconciliation().ready()).isTrue();

		var activeReplay = migrationService.importManifest(cutoverKey, BUSINESS_DATE, WRITER_VERSION,
	...
	private OrchidGroupStateChainMigrationManifest manifest(Long currentGroupId, OrchidGroupStateSnapshot observed,
			OrchidGroupStateSnapshot current, OrchidGroupStateSnapshot deleted) {
		return new OrchidGroupStateChainMigrationManifest(2, "orchid_state_chain_manifest_normalizer", true, List.of(),
				Map.of(), Map.of(), List.of(
						mutation("current-baseline", OrchidGroupMutationType.BASELINE_IMPORT,
								"EARLIEST_TRUSTWORTHY_BASELINE", "current-observation",
								entry(currentGroupId, OrchidGroupMutationEntryKind.BASELINE,
										OrchidGroupMutationEntryRole.AFFECTED, null, 0L, null, observed)),
						mutation("current-correction", OrchidGroupMutationType.CORRECTION, "AUDIT_EVENT",
								"current-correction",
								entry(currentGroupId, OrchidGroupMutationEntryKind.CHANGE,
										OrchidGroupMutationEntryRole.AFFECTED, 0L, 1L, observed, current)),
						mutation("deleted-create", OrchidGroupMutationType.CREATE, "AUDIT_EVENT", "deleted-create",
								entry(DELETED_GROUP_ID, OrchidGroupMutationEntryKind.CREATE,
										OrchidGroupMutationEntryRole.RESULT, null, 1L, null, deleted)),
						mutation("deleted-terminal", OrchidGroupMutationType.DELETE, "AUDIT_EVENT", "deleted-terminal",
								entry(DELETED_GROUP_ID, OrchidGroupMutationEntryKind.DELETE,
										OrchidGroupMutationEntryRole.SOURCE, 1L, 2L, deleted, null))));
	}

	private OrchidGroupStateChainMigrationManifest.Mutation mutation(String key, OrchidGroupMutationType type,
			String sourceType, String sourceReference, OrchidGroupStateChainMigrationManifest.Entry entry) {
		return new OrchidGroupStateChainMigrationManifest.Mutation(key, type, sourceType, sourceReference,
				Instant.parse("2026-08-20T00:00:00Z"), BUSINESS_DATE, "PostgreSQL state-chain rehearsal", Map.of(),
				List.of(entry));
	}

	private OrchidGroupStateChainMigrationManifest.Entry entry(Long groupId, OrchidGroupMutationEntryKind kind,
			OrchidGroupMutationEntryRole role, Long revisionBefore, Long revisionAfter, OrchidGroupStateSnapshot before,
			OrchidGroupStateSnapshot after) {
		return new OrchidGroupStateChainMigrationManifest.Entry(groupId, kind, role, revisionBefore, revisionAfter,
				before, after);
	}

	private OrchidGroupStateSnapshot withQuantity(OrchidGroupStateSnapshot source, int quantity) {
		return new OrchidGroupStateSnapshot(quantity, source.reservedQuantity(), source.status(), source.bedZoneId(),
				source.sortOrder(), source.startPosition(), source.endPosition(), source.varietyId(), source.genus(),
				source.varietyName(), source.ageYear(), source.potSizeCode(), source.placementType(),
				source.trayCount(), source.splitPlacementAllowed(), source.inboundRecordId(), source.memo());
	}
