...
class WorkEffectResultsTest {
	...
	void creationAndPottingKeepTheirDistinctCountKeys() {
		assertThat(new WorkEffectResults.Created(List.of(3L, 4L)).toMap())
			.isEqualTo(Map.of("createdCount", 2, "createdOrchidGroupIds", List.of(3L, 4L)));
		assertThat(new WorkEffectResults.Potted(9L, List.of(3L, 4L), 20).toMap()).isEqualTo(Map.of("inboundRecordId",
				9L, "createdOrchidGroupIds", List.of(3L, 4L), "actualQuantity", 20, "resultCount", 2));
	}

	@Test
	void movementKeepsRawSnapshotAndNullPositions() {
		assertThat(new WorkEffectResults.Moved(1L, "old-zone", 9L, null, null).toMap())
			.containsEntry("fromBedZoneId", "old-zone")
			.containsEntry("startPosition", null)
			.containsEntry("endPosition", null);
	}

	@Test
	void discardOmitsMissingReasonAndTrimsPresentReason() {
		assertThat(new WorkEffectResults.Discarded(1L, 10, 2, 8, "관리", "관리", " ").toMap()).doesNotContainKey("reason")
			.containsEntry("discardedQuantity", 2)
			.containsEntry("remainingQuantity", 8);
		assertThat(new WorkEffectResults.Discarded(1L, 10, 2, 8, "관리", "관리", " 사유 ").toMap()).containsEntry("reason",
				"사유");
	}
	...
	void correctionKeepsDatesAndAdjustmentFields() {
		...
		assertThat(new WorkEffectResults.Corrected(1L, date, date.minusDays(1),
				List.of(new WorkEffectResults.Adjustment(3L, 10, "관리", 8, "정상")))
			.toMap()).containsEntry("beforeWorkDate", date)
			.containsEntry("afterWorkDate", date.minusDays(1))
			.containsEntry("adjustments", List.of(Map.of("orchidGroupId", 3L, "beforeQuantity", 10, "beforeStatus",
					"관리", "afterQuantity", 8, "afterStatus", "정상")));
	}
	...
	void lineageKeepsNumericOnlyIdsAndLegacyMergeQuantity() {
		...
		assertThat(WorkEffectResults.sourceQuantities(Map.of("sources",
				List.of(Map.of("sourceOrchidGroupId", "1", "inputQuantity", 4),
