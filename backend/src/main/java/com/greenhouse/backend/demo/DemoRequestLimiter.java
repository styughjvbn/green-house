...
/** Per-process, per-client fixed UTC windows. Rejected minute requests do not consume the daily quota. */
...
final class DemoRequestLimiter {
	...
	DemoRequestLimit record(String client, boolean mutation) {
		...
		RequestWindow window = windows.compute(client, (key, previous) ->
				previous == null || previous.minute() != currentMinute
						? new RequestWindow(currentMinute, 1, mutation ? 1 : 0)
						: previous.record(mutation));
	...
	private int recordDailyMutation(String client) {
		...
		return dailyWindows.compute(client, (key, previous) ->
				previous == null || previous.day() != currentDay
						? new DailyMutationWindow(currentDay, 1)
						: new DailyMutationWindow(currentDay, previous.mutations() + 1)).mutations();
	}
