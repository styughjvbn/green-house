...
public class WorkTargetExecutionRepositoryImpl implements WorkTargetExecutionRepositoryCustom {
	...
	public List<WorkTargetExecution> findActiveInboundPottingForUpdate(Collection<Long> inboundRecordIds) {
		return executionWithOperationAndWorkType().where(workOperationTarget.inboundRecordId.in(inboundRecordIds),
				workType.code.eq(WorkTypeDefinition.POTTING.name()),
				workOperation.status.in(WorkOperationStatus.PLANNED, WorkOperationStatus.IN_PROGRESS,
						WorkOperationStatus.PAUSED),
				workTargetExecution.status.in(WorkTargetExecutionStatus.PENDING, WorkTargetExecutionStatus.IN_PROGRESS))
			.orderBy(workOperationTarget.inboundRecordId.asc(), workOperation.plannedStartDate.asc(),
	...
	public List<WorkTargetExecution> findForUpdateByTargetInboundRecordIdOrderByIdAsc(Long inboundRecordId) {
		return executionWithOperationAndWorkType().where(workOperationTarget.inboundRecordId.eq(inboundRecordId))
	...
	public List<WorkTargetExecution> findByTargetWorkOperationIdOrderByIdAsc(Long workOperationId) {
		return queryFactory.selectFrom(workTargetExecution)
			.join(workTargetExecution.target, workOperationTarget)
			.fetchJoin()
	...
	@Override
	public List<WorkTargetExecution> findByTargetWorkOperationIdInOrderByIdAsc(Collection<Long> workOperationIds) {
		return queryFactory.selectFrom(workTargetExecution)
			.join(workTargetExecution.target, workOperationTarget)
			.fetchJoin()
	...
	@Override
	public List<WorkOperationProgressProjection> findProgressByWorkOperationIdIn(Collection<Long> workOperationIds) {
		...
		List<Tuple> rows = queryFactory
			.select(workOperationTarget.workOperation.id, workOperationTarget.id.count(), pending, inProgress, partial,
					completed, skipped, canceled, failed, totalQuantity, processedQuantity, skippedQuantity)
			.from(workOperationTarget)
			.join(workTargetExecution)
			.on(workTargetExecution.target.eq(workOperationTarget))
			.where(workOperationTarget.workOperation.id.in(workOperationIds), workOperationTarget.excludedAt.isNull())
			.groupBy(workOperationTarget.workOperation.id)
			.fetch();
		return rows.stream()
			.map(row -> new WorkOperationProgressProjection(row.get(workOperationTarget.workOperation.id),
					intValue(row.get(workOperationTarget.id.count())), intValue(row.get(pending)),
					intValue(row.get(inProgress)), intValue(row.get(partial)), intValue(row.get(completed)),
					intValue(row.get(skipped)), intValue(row.get(canceled)), intValue(row.get(failed)),
					intValue(row.get(totalQuantity)), intValue(row.get(processedQuantity)),
					intValue(row.get(skippedQuantity))))
			.toList();
	}

	@Override
	public List<WorkTargetExecution> findForUpdateByTargetWorkOperationIdOrderByIdAsc(Long workOperationId) {
		return executionWithOperationAndWorkType().where(workOperation.id.eq(workOperationId))
			.orderBy(workTargetExecution.id.asc())
			.setLockMode(LockModeType.PESSIMISTIC_WRITE)
			.fetch();
	}
	...
	@Override
	public Optional<WorkTargetExecution> findByTargetIdAndTargetWorkOperationId(Long targetId, Long workOperationId) {
		return Optional.ofNullable(executionWithOperation()
			.where(workOperationTarget.id.eq(targetId), workOperation.id.eq(workOperationId))
			.fetchOne());
	}

	@Override
	public Optional<WorkTargetExecution> findForUpdateByTargetIdAndTargetWorkOperationId(Long targetId,
	...
	private JPAQuery<WorkTargetExecution> executionWithOperationAndWorkType() {
		return executionWithOperation().join(workOperation.workType, workType).fetchJoin();
	}

	private NumberExpression<Integer> countStatus(WorkTargetExecutionStatus status) {
		return new CaseBuilder().when(workTargetExecution.status.eq(status)).then(1).otherwise(0).sum();
	}
