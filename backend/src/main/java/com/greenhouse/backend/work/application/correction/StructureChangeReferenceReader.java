...
public class StructureChangeReferenceReader {
	...
	public Set<Long> getActiveOrchidGroupIds(Long workOperationId) {
		return workOperationTargetRepository.findByWorkOperationIdAndExcludedAtIsNullOrderByIdAsc(workOperationId)
			.stream()
	...
	public StructureChangeMutationReferences getMutationReferences(Long operationId, Set<Long> orchidGroupIds) {
		...
		return StructureChangeMutationReferences.current(
				links.stream().map(link -> link.getWorkAppliedEffect().getMutationId()).distinct().sorted().toList());
	}
