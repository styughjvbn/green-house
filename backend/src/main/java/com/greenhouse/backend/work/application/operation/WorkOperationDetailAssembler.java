...
final class WorkOperationDetailAssembler {
	...
	private static final Set<String> HIDDEN_FIELD_KEYS = Set.of("inboundRecordId", "orchidGroupId", "requestKey",
			"idempotencyKey");

	private static final Map<String, String> FIELD_LABELS = Map.ofEntries(Map.entry("actualQuantity", "실제 수량"),
			Map.entry("ageYear", "년생"), Map.entry("bottleCount", "병 수"), Map.entry("createdCount", "생성된 난 묶음 수"),
			Map.entry("dilutionRatio", "희석 배수"), Map.entry("estimatedQuantity", "예상 수량"), Map.entry("genus", "속명"),
			Map.entry("growthStage", "생육 단계"), Map.entry("inboundType", "입고 유형"), Map.entry("materialName", "자재명"),
			Map.entry("originalWorkOperationId", "원본 작업"), Map.entry("placementType", "배치 규격"),
			Map.entry("potSize", "화분 크기"), Map.entry("pottingDueDate", "포트 예정일"), Map.entry("quantity", "사용량"),
			Map.entry("reason", "보정 사유"), Map.entry("resultCount", "결과 난 묶음 수"), Map.entry("rowCount", "생성 예정 건수"),
			Map.entry("status", "상태"), Map.entry("tempLocation", "임시 위치"), Map.entry("trayCount", "판수"),
	...
	static List<WorkOperationDetailFieldResponse> fields(WorkOperation operation) {
		...
		return operation.getDetails()
			...
			.map(entry -> new WorkOperationDetailFieldResponse(entry.getKey(), fieldLabel(operation, entry.getKey()),
					formatValue(entry.getValue())))
			.toList();
	}

	private static boolean isHiddenFieldKey(String key) {
		return HIDDEN_FIELD_KEYS.contains(key) || key.equals("migrationSource") || key.startsWith("legacy");
	}
	...
	private static boolean isDisplayValue(Object value) {
		return value != null && (!(value instanceof String text) || !text.isBlank()) && !(value instanceof Map<?, ?>)
