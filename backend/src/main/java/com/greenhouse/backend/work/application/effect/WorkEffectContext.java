...
public record WorkEffectContext(Long operationId, String workTypeCode, LocalDate plannedStartDate, String memo,
		Target target) {
	...
	static WorkEffectContext from(WorkOperation operation, WorkOperationTarget target) {
		return new WorkEffectContext(operation.getId(), operation.getWorkType().getCode(),
				operation.getPlannedStartDate(), operation.getMemo(),
				target == null ? null : new Target(target.getTargetReferenceType(), target.getOrchidGroupId(),
						target.getInboundRecordId(), target.getLocationSnapshot()));
	}

	public record Target(WorkTargetReferenceType referenceType, Long orchidGroupId, Long inboundRecordId,
