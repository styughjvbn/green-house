...
public record BusinessPartnerResponse(
	...
	public static BusinessPartnerResponse from(BusinessPartnerInfo partner) {
		return new BusinessPartnerResponse(partner.id(), partner.name(), partner.partnerType(),
				partner.ownerName(), partner.phone(), partner.address(), partner.memo(), partner.active());
	}
