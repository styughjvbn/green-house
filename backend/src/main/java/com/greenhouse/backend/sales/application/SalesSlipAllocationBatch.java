...
public record SalesSlipAllocationBatch(
	...
	public void captureSnapshot(SalesOrchidSnapshotType snapshotType, LocalDateTime capturedAt,
		...
		lines.forEach(line -> captureSnapshot(line.allocation(), snapshotType, capturedAt, states.get(line.orchidGroupId())));
	}

	static SalesOrchidGroupSnapshot captureSnapshot(SalesSlipItemAllocation allocation,
			SalesOrchidSnapshotType type, LocalDateTime capturedAt, OrchidGroupState state) {
		return allocation.captureSnapshot(SalesOrchidGroupSnapshot.builder()
				.snapshotType(type).captureSource(SalesOrchidSnapshotSource.LIVE).capturedAt(capturedAt)
				.orchidGroupId(state.id()).allocatedQuantity(allocation.getAllocatedQuantity())
				.varietyId(state.varietyId()).varietyName(state.varietyName()).genus(state.genus())
				.ageYear(state.ageYear()).potSizeCode(state.potSizeCode()).potSize(state.potSize())
				.quantity(state.quantity()).reservedQuantity(state.reservedQuantity()).status(state.status())
				.houseId(state.houseId()).houseNumber(state.houseNumber())
				.physicalBedId(state.physicalBedId()).physicalBedNumber(state.physicalBedNumber())
				.bedZoneId(state.bedZoneId()).bedZoneName(state.bedZoneName())
				.startPosition(state.startPosition()).endPosition(state.endPosition()).build());
	}
