...
public class SalesSlipAuditSupport {
		...
		data.put("items",
				slip.getItems()
					...
					.map(item -> {
						...
						value.put("allocations",
								item.getAllocations()
									.stream()
									.sorted(Comparator.comparing(allocation -> allocation.getOrchidGroupId()))
									.map(allocation -> Map.of("orchidGroupId", allocation.getOrchidGroupId(),
	...
	public Long record(AuditAction action, SalesSlip slip, Map<String, Object> before, Map<String, Object> after) {
		return auditWriter.record(action, AuditSource.SALES_MANAGEMENT, "SALES_SLIP", slip.getId(), before, after,
