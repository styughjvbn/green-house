...
public class SalesMetricsReader {
	...
	public Map<YearMonth, Long> monthlySales(LocalDate from, LocalDate to) {
		...
		return queryFactory.select(year, month, amount)
			...
			.fetch()
			.stream()
			.collect(Collectors.toUnmodifiableMap(row -> YearMonth.of(row.get(year), row.get(month)),
					row -> row.get(amount)));
	}
	...
	public Map<Long, PartnerSales> partnerSales(LocalDate from, LocalDate to) {
		return queryFactory
			.select(Projections.constructor(PartnerSales.class, salesSlip.partnerId,
					salesSlip.totalAmount.sum().longValue(), salesSlip.id.count(),
	...
	public Map<SalesPaymentCategory, Long> paymentBreakdown(LocalDate from, LocalDate to) {
		return queryFactory
			.select(Projections.constructor(NamedAmount.class, salesSlip.paymentStatus,
			...
			.fetch()
			.stream()
			.collect(Collectors.toUnmodifiableMap(row -> SalesPaymentCategory.fromStoredStatus(row.name()),
					NamedAmount::amount, Long::sum));
	}
	...
	private JPAQuery<SlipSummary> slipSummaryQuery(LocalDate from, LocalDate to) {
		return queryFactory
			.select(Projections.constructor(SlipSummary.class, salesSlip.id, salesSlip.slipNumber, salesSlip.saleDate,
					salesSlip.partnerId, salesSlip.totalAmount, salesSlip.paidAmount, salesSlip.remainingAmount,
					salesSlip.paymentStatus, salesSlip.salesStatus))
			.from(salesSlip)
			.where(completedInPeriod(from, to));
	}
	...
	private BooleanExpression completedInPeriod(LocalDate from, LocalDate to) {
		return salesSlip.saleDate.between(from, to)
			.and(salesSlip.salesStatus.in(SalesSlip.STATUS_DIRECT_OUTBOUND_COMPLETED,
	...
	public record PartnerSales(Long partnerId, long totalSales, long transactionCount, long unpaidAmount,
			long paidAmount, LocalDate latestSaleDate) {
	}
