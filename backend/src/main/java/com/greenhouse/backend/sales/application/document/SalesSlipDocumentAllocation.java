...
public record SalesSlipDocumentAllocation(Long id, Long orchidGroupId, String varietyName, Integer allocatedQuantity,
		Integer availableQuantity, Integer houseNumber, Integer physicalBedNumber, String bedZoneName,
		SalesOrchidGroupSnapshotData creationSnapshot, SalesOrchidGroupSnapshotData outboundSnapshot) {

	public static SalesSlipDocumentAllocation from(SalesSlipItemAllocation allocation, OrchidGroupState group) {
		return new SalesSlipDocumentAllocation(allocation.getId(), group.id(), group.varietyName(),
				allocation.getAllocatedQuantity(), group.availableQuantity(), group.houseNumber(),
				group.physicalBedNumber(), group.bedZoneName(),
