...
public record AuctionShipmentOptionResponse(Long id, LocalDate shipmentDate, Long auctionHouseId, String auctionMarket,
		List<Lot> lots) {

	public static AuctionShipmentOptionResponse from(Shipment shipment) {
		return new AuctionShipmentOptionResponse(shipment.id(), shipment.shipmentDate(), shipment.auctionHouseId(),
				shipment.auctionMarket(),
				shipment.lots()
					.stream()
					.map(lot -> new Lot(lot.id(), lot.itemName(), lot.varietyName(), lot.shipmentGrade(),
							lot.shippedQuantity()))
					.toList());
	}
