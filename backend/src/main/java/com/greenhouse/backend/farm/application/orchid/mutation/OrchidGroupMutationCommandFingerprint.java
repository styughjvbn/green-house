...
public class OrchidGroupMutationCommandFingerprint {
	...
	public String calculate(OrchidGroupMutationCommand command) {
		return switch (command) {
			case CreateOrchidGroupMutationCommand value -> fingerprint.calculate(new CreatePayload(
					...
					value.reason()));
			case CreateOrchidGroupsMutationCommand value -> fingerprint.calculate(new CreateManyPayload(
					OrchidGroupMutationType.CREATE,
					value.groups(),
					value.effectiveBusinessDate(),
					value.reason()));
			case CreateInboundOrchidGroupsMutationCommand value -> fingerprint.calculate(new CreateInboundPayload(
					OrchidGroupMutationType.CREATE,
					value.inboundRecordId(),
					value.groups(),
					value.effectiveBusinessDate(),
					value.reason()));
			case TransformOrchidGroupsMutationCommand value -> fingerprint.calculate(new TransformPayload(
					OrchidGroupMutationType.TRANSFORM,
					value.sources(),
					value.results(),
					value.effectiveBusinessDate(),
					value.reason(),
					value.placementExclusionOrchidGroupIds()));
			case UpdateOrchidGroupMutationCommand value -> fingerprint.calculate(new UpdatePayload(
					OrchidGroupMutationType.UPDATE_DETAILS,
					value.orchidGroupId(),
					value.details(),
					value.effectiveBusinessDate(),
					value.reason()));
			case MoveOrchidGroupMutationCommand value -> fingerprint.calculate(new MovePayload(
					...
					value.effectiveBusinessDate(),
					value.reason()));
			case CancelOrchidGroupCreationMutationCommand value -> fingerprint.calculate(new CancelCreationPayload(
					OrchidGroupMutationType.CANCEL_CREATION,
					value.orchidGroupId(),
					value.effectiveBusinessDate(),
					value.reason()));
			case DiscardOrchidGroupMutationCommand value -> fingerprint.calculate(new DiscardPayload(
					OrchidGroupMutationType.DISCARD,
					value.orchidGroupId(),
					value.quantity(),
					value.effectiveBusinessDate(),
					value.reason()));
			case ReserveOrchidGroupsMutationCommand value -> quantity(
					OrchidGroupMutationType.RESERVE,
					value.items(),
					null,
					value.effectiveBusinessDate(),
					value.reason());
			case ReleaseOrchidGroupReservationsMutationCommand value -> quantity(
					OrchidGroupMutationType.RELEASE_RESERVATION,
					value.items(),
					null,
					value.effectiveBusinessDate(),
					value.reason());
			case ConsumeOrchidGroupReservationsMutationCommand value -> quantity(
					OrchidGroupMutationType.CONSUME_RESERVATION,
					value.items(),
					null,
					value.effectiveBusinessDate(),
					value.reason());
			case RestoreOutboundOrchidGroupsMutationCommand value -> quantity(
					OrchidGroupMutationType.RESTORE_OUTBOUND,
					value.items(),
					value.compensatedMutations(),
					value.effectiveBusinessDate(),
					value.reason());
			case CorrectOrchidGroupsMutationCommand value -> fingerprint.calculate(new CorrectionPayload(
	...
	private String quantity(
			...
			String reason) {
		return fingerprint.calculate(new QuantityPayload(
				mutationType, items, relatedMutations, effectiveBusinessDate, reason));
	}
	...
	private record CreateManyPayload(
			OrchidGroupMutationType mutationType,
