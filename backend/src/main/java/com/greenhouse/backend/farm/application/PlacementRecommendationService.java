...
public class PlacementRecommendationService {
	...
	public PlacementRecommendationService(OrchidGroupRepository orchidGroupRepository, BedZoneRepository bedZoneRepository, BedZoneSegmentRepository segmentRepository, OrchidGroupSegmentPlacementRepository placementRepository) {
	...
	public PlacementRecommendationResponse recommend(Long orchidGroupId, Long houseId) {
		...
		return new PlacementRecommendationResponse(
			...
			new PlacementRequirementResponse(placementType, group.getPotSize(), group.getQuantity(), requiredUnits, group.getSplitPlacementAllowed()),
			candidates
		);
	}
	...
	public List<OrchidGroupSegmentPlacement> validateAndCreatePlacements(
		...
		if (mode == null) throw new IllegalArgumentException("배치 모드는 필수입니다.");
		if (mode == PlacementCapacityMode.TEMPORARY && reorganizeDueDate == null) {
			throw new IllegalArgumentException("임시 배치는 재정리 예정일이 필요합니다.");
		}
		if (requests == null || requests.isEmpty()) throw new IllegalArgumentException("구간별 배치 정보가 필요합니다.");
		if (!group.getSplitPlacementAllowed() && requests.size() > 1) throw new IllegalArgumentException("분할 배치가 허용되지 않은 난 묶음입니다.");
		String placementType = requirePlacementType(group);
		...
		for (var request : requests) {
			if (!segmentIds.add(request.segmentId())) throw new IllegalArgumentException("같은 구간을 중복 지정할 수 없습니다.");
			...
			if (!segment.getBedZone().getId().equals(destination.getId())) throw new IllegalArgumentException("목적 구역에 속하지 않은 구간입니다.");
			...
			if (units > available) throw new CapacityConflictException(segment.getName() + "의 남은 수용량이 부족합니다.");
			totalQuantity += request.quantity();
			totalUnits += units;
			result.add(new OrchidGroupSegmentPlacement(segment, request.quantity(), request.trayCount(), mode, reorganizeDueDate, memo));
		}
		if (totalQuantity != group.getQuantity()) throw new IllegalArgumentException("구간별 수량 합계가 난 묶음 수량과 다릅니다.");
		if (totalUnits != requiredUnits(group)) throw new IllegalArgumentException("구간별 점유 단위 합계가 난 묶음 점유량과 다릅니다.");
		return result;
	}

	private PlacementRecommendationCandidateResponse evaluateZone(OrchidGroup group, BedZone zone, String placementType, int requiredUnits) {
		if (zone.getOrchidGroups().stream().anyMatch(item -> !item.getId().equals(group.getId()) && item.getSegmentPlacements().isEmpty())) {
		...
		if (segments.isEmpty()) return unavailable(zone, "배드 구간 설정이 없습니다.");
		for (PlacementCapacityMode mode : PlacementCapacityMode.values()) {
			AllocationResult allocation = allocate(segments, group, placementType, group.getPotSize(), mode, requiredUnits);
	...
	private AllocationResult allocate(List<BedZoneSegment> segments, OrchidGroup group, String placementType, String potSize, PlacementCapacityMode mode, int requiredUnits) {
		...
		for (BedZoneSegment segment : segments) {
			...
			collectWarnings(segment, placementType, potSize, mode, warnings);
			remaining -= allocated;
			if (remaining == 0) return new AllocationResult(true, normalizeQuantities(allocations, group.getQuantity()), warnings.stream().distinct().toList());
		}
		return new AllocationResult(false, List.of(), List.of());
	}

	private PlacementRecommendationAllocationResponse allocation(BedZoneSegment segment, OrchidGroup group, int units, int totalUnits, int available) {
		int quantity = Math.max(1, (int) Math.floor((double) group.getQuantity() * units / totalUnits));
		return new PlacementRecommendationAllocationResponse(segment.getId(), segment.getName(), quantity, units, available - units);
	}

	private List<PlacementRecommendationAllocationResponse> normalizeQuantities(List<PlacementRecommendationAllocationResponse> allocations, int totalQuantity) {
		...
		if (difference == 0 || allocations.isEmpty()) return allocations;
		...
		adjusted.set(adjusted.size() - 1, new PlacementRecommendationAllocationResponse(
			last.segmentId(), last.segmentName(), last.quantity() + difference, last.occupancyUnits(), last.remainingUnits()
		));
		return adjusted;
	}

	private int availableUnits(BedZoneSegment segment, OrchidGroup excludedGroup, String placementType, String potSize, PlacementCapacityMode mode) {
		BedZoneSegmentCapacity target = findCapacity(segment, placementType, potSize, mode);
		if (target == null || !target.getAllowed() || target.getCapacityValue() <= 0) return 0;
		double occupiedRatio = 0;
		for (OrchidGroupSegmentPlacement placement : placementRepository.findBySegmentId(segment.getId())) {
			if (placement.getOrchidGroup().getId().equals(excludedGroup.getId())) continue;
			OrchidGroup occupiedGroup = placement.getOrchidGroup();
			String occupiedType = normalizeType(occupiedGroup.getPlacementType());
			if (occupiedType == null) return 0;
			BedZoneSegmentCapacity occupiedCapacity = findCapacity(segment, occupiedType, occupiedGroup.getPotSize(), mode);
			if (occupiedCapacity == null || !occupiedCapacity.getAllowed() || occupiedCapacity.getCapacityValue() <= 0) return 0;
			int occupiedUnits = usesTrayUnits(occupiedType) ? Objects.requireNonNullElse(placement.getTrayCount(), 0) : placement.getQuantity();
	...
	private BedZoneSegmentCapacity findCapacity(BedZoneSegment segment, String placementType, String potSize, PlacementCapacityMode mode) {
		return segment.getCapacities().stream()
			.filter(capacity -> capacity.getPlacementType().equals(placementType) && capacity.getCapacityMode() == mode)
			.filter(capacity -> Objects.equals(capacity.getPotSize(), potSize))
			.findFirst()
			.orElseGet(() -> segment.getCapacities().stream()
				.filter(capacity -> capacity.getPlacementType().equals(placementType) && capacity.getCapacityMode() == mode && capacity.getPotSize() == null)
				.findFirst().orElse(null));
	}

	private void collectWarnings(BedZoneSegment segment, String placementType, String potSize, PlacementCapacityMode mode, List<String> warnings) {
		if (segment.getMemo() != null) warnings.add(segment.getName() + ": " + segment.getMemo());
		BedZoneSegmentCapacity capacity = findCapacity(segment, placementType, potSize, mode);
		if (capacity != null && capacity.getMemo() != null) warnings.add(segment.getName() + ": " + capacity.getMemo());
	}
	...
	private PlacementRecommendationCandidateResponse candidate(BedZone zone, PlacementRecommendationStatus status, PlacementCapacityMode mode, List<PlacementRecommendationAllocationResponse> allocations, List<String> warnings) {
		var bed = zone.getPhysicalBed(); var house = bed.getHouse();
		return new PlacementRecommendationCandidateResponse(zone.getId(), zone.getName(), house.getId(), house.getNumber(), bed.getId(), bed.getNumber(), status, mode, allocations, warnings);
	}
	...
	private Comparator<PlacementRecommendationCandidateResponse> candidateComparator() {
		return Comparator.comparing((PlacementRecommendationCandidateResponse value) -> value.status().ordinal())
			.thenComparing(value -> value.requiredMode() == null ? Integer.MAX_VALUE : value.requiredMode().ordinal())
	...
	private OrchidGroup findGroup(Long id) { return orchidGroupRepository.findById(id).orElseThrow(() -> new NotFoundException("난 묶음을 찾을 수 없습니다.")); }
	private String requirePlacementType(OrchidGroup group) { String value = normalizeType(group.getPlacementType()); if (value == null) throw new IllegalArgumentException("추천 전에 난 묶음의 배치 규격을 지정해야 합니다."); return value; }
	private String normalizeType(String value) { if (value == null || value.isBlank()) return null; return value.trim().toUpperCase(); }
	private boolean usesTrayUnits(String type) { return type.startsWith("TRAY_") || type.startsWith("CUSTOM:"); }
	private int requiredUnits(OrchidGroup group) { String type = requirePlacementType(group); return usesTrayUnits(type) ? requireTrayCount(group.getTrayCount()) : group.getQuantity(); }
	private int requireTrayCount(Integer value) { if (value == null || value < 1) throw new IllegalArgumentException("판 배치는 실제 판 수가 필요합니다."); return value; }

	private record AllocationResult(boolean complete, List<PlacementRecommendationAllocationResponse> allocations, List<String> warnings) { }
}
