...
/**
 * ORCHID-CUTOVER: LEGACY_RETIRE — Engine 경로와 전환 후 제거할 직접 합식 분기를 함께 가진다. Removal gate: 운영
...
public class MergeWorkHandler implements WorkEffectHandler {
	...
	public WorkExecutionResult execute(WorkEffectContext context, WorkEffectCommand command) {
		...
		List<Long> sourceIds = request.sources()
			...
			.sorted()
			.toList();
		Map<Long, OrchidGroup> sourcesById = orchidGroupRepository.findAllForUpdateByIdIn(sourceIds)
			.stream()
		...
		Map<Long, Integer> inputBySourceId = request.sources()
			.stream()
			.collect(Collectors.toMap(MergeSourceInputRequest::sourceOrchidGroupId,
		...
		inputBySourceId.forEach((sourceId, inputQuantity) -> sourcesById.get(sourceId).applyRepot(inputQuantity));
		OrchidGroup result = orchidGroupCommandService.createEntity(new OrchidGroupCreateRequest(row.bedZoneId(),
				varietyId, row.quantity(), row.potSize(), row.ageYear(), resultStatus, row.placementType(),
				row.trayCount(), row.splitPlacementAllowed(), row.startPosition(), row.endPosition(), row.memo()));
		var details = new WorkEffectResults.Merged(sourceIds, inputBySourceId, totalInput, lossQuantity, result.getId())
			.toMap();
