...
public class DerivedOrchidGroupService {
	...
	public List<DerivedOrchidGroupResponse> getGroups(Long varietyId, Integer ageYear, PotSizeCode potSizeCode,
			Long houseId, String status, String keyword) {
	...
	public List<OrchidGroupResponse> getMembers(String groupKey, Long houseId, String status, String keyword) {
		var businessDate = TimeConfig.farmToday(clock);
	...
	private List<OrchidGroup> findCandidates(Long varietyId, PotSizeCode potSizeCode, Long houseId, String status,
			String keyword) {
		if (potSizeCode == PotSizeCode.UNMAPPED) {
			return List.of();
		}
		return orchidGroupRepository.findDerivedGroupCandidates(varietyId, potSizeCode, houseId, normalize(status),
				normalize(keyword));
	}
	...
	private DerivedOrchidGroupResponse toResponse(GroupKey key, List<OrchidGroupResponse> members) {
		...
		return new DerivedOrchidGroupResponse(key.serialize(), key.varietyId(), first.varietyName(), first.genus(),
				key.ageYear(), key.potSizeCode(), first.potSize(), members.size(),
