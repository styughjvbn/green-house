...
public class OrchidGroupLedgerCutoverService {
	...
	public OrchidGroupLedgerCutoverResult execute(OrchidGroupLedgerCutoverCommand command) {
		...
		if (initialReport.stage() == OrchidGroupLedgerReconciliationStage.ACTIVE) {
			preparationService.validatePreparation(command.cutoverKey(), command.effectiveBusinessDate(),
		...
		if (!finalReport.ready() || finalReport.stage() != OrchidGroupLedgerReconciliationStage.BASELINE_PREPARING
		...
		if (command.activate()) {
			finalReport = preparationService.activate(command.cutoverKey(), command.currentWriterVersion());
		}
		return result(command, finalReport);
	}

	private void validateInitialState(OrchidGroupLedgerCutoverCommand command,
		...
		if (!report.ready()) {
			throw new ConflictException("Cutover 전에 해결할 대사 오류가 있습니다: " + report.issues().getFirst().code());
		}
		preparationService.validateStateChainImported(command.cutoverKey());
	}

	private OrchidGroupLedgerCutoverResult result(OrchidGroupLedgerCutoverCommand command,
			OrchidGroupLedgerReconciliationReport report) {
		return new OrchidGroupLedgerCutoverResult(command.cutoverKey(), command.effectiveBusinessDate(),
				command.minimumWriterVersion(), command.currentWriterVersion(), report.baselineGroupCount(),
				report.stage() == OrchidGroupLedgerReconciliationStage.ACTIVE, report);
	}
