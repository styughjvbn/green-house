...
public record CreateInboundOrchidGroupsMutationCommand(OrchidGroupMutationSource source, Long inboundRecordId,
	...
	public CreateInboundOrchidGroupsMutationCommand {
		if (source == null || inboundRecordId == null || effectiveBusinessDate == null) {
			throw new IllegalArgumentException("입고 생성 Mutation의 source, 입고 기록과 업무일이 필요합니다.");
		}
		if (groups == null || groups.isEmpty() || groups.stream().anyMatch(item -> item == null)) {
			throw new IllegalArgumentException("입고에서 생성할 난 묶음이 필요합니다.");
		}
