...
/**
 * ORCHID-CUTOVER: LEGACY_RETIRE — Engine 경로와 전환 후 제거할 직접 writer를 함께 가진다. Removal gate: 운영
...
public class OrchidGroupCommandService {
	...
	public OrchidGroupResponse create(OrchidGroupCreateRequest request) {
		...
		var command = mutationRoutingPolicy.routesToEngine() ? new CreateOrchidGroupMutationCommand(
				OrchidGroupMutationSources.farmRequest("ORCHID_GROUP_COMMAND", "DIRECT", "CREATE"), request.bedZoneId(),
				mutationDetails(request), businessDate, "난 묶음 등록") : null;
		OrchidGroup created = mutationRoutingPolicy.routesToEngine() ? createWithEngine(command)
	...
	public OrchidGroup createEntity(OrchidGroupCreateRequest request, Set<Long> placementExclusionOrchidGroupIds) {
		...
		orchidPlacementPolicy.validatePlacementExcluding(bedZone, startPosition, endPosition,
				placementExclusionOrchidGroupIds);

		int nextSortOrder = orchidGroupRepository.findMaxSortOrderByBedZoneId(bedZone.getId()) + 1;
		OrchidGroup orchidGroup = new OrchidGroup(bedZone, variety.getGenus(), variety.getName(), request.quantity(),
				normalize(request.potSize()), request.ageYear(), normalizeRequired(request.status()), nextSortOrder,
				startPosition, endPosition);
		orchidGroup.updateDetails(variety.getGenus(), variety.getName(), request.quantity(),
				normalize(request.potSize()), request.ageYear(), normalizeRequired(request.status()),
				normalize(request.placementType()), request.trayCount(), request.splitPlacementAllowed(), startPosition,
				endPosition, normalize(request.memo()));
	...
	private OrchidGroupResponse update(Long orchidGroupId, OrchidGroupUpdateRequest request, String correctionMode) {
		...
		OrchidGroupMutationDetails details = mutationRoutingPolicy.routesToEngine() ? mutationDetails(request) : null;
	...
	public void delete(Long orchidGroupId) {
		...
		var command = mutationRoutingPolicy
			.routesToEngine()
					? new CancelOrchidGroupCreationMutationCommand(
							OrchidGroupMutationSources.farmRequest("ORCHID_GROUP_COMMAND", orchidGroupId.toString(),
									"CANCEL_CREATION"),
							orchidGroupId, TimeConfig.farmToday(clock), "난 묶음 삭제 요청에 따른 생성 취소")
					: null;
		if (mutationRoutingPolicy.routesToEngine()) {
		...
		auditSupport.record(orchidGroupId, AuditAction.DELETED, AuditSource.ORCHID_GROUP_MANAGEMENT, before, null,
				Map.of());
	}
	...
	private OrchidGroupResponse moveForOperation(Long orchidGroupId, OrchidGroupMoveRequest request,
		...
		if (fromBedZoneId.equals(toBedZone.getId()) && equalPosition(orchidGroup.getStartPosition(), startPosition)
				&& equalPosition(orchidGroup.getEndPosition(), endPosition)) {
			return OrchidGroupResponse.from(orchidGroup, businessDate);
		}
		var command = applyRouting && mutationRoutingPolicy.routesToEngine() ? new MoveOrchidGroupMutationCommand(
				OrchidGroupMutationSources.farmRequest("ORCHID_GROUP_COMMAND", orchidGroupId.toString(), "MOVE"),
				orchidGroupId, request.toBedZoneId(), startPosition, endPosition, businessDate, request.memo()) : null;
