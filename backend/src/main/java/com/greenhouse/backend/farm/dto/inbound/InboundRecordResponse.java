...
public record InboundRecordResponse(Long id, LocalDate inboundDate, String inboundType, Long varietyId, String genus,
		String varietyName, String status, Integer bottleCount, Integer estimatedQuantity, Integer actualQuantity,
		String tempLocation, LocalDate pottingDueDate, LocalDate pottingDate, String potSize, Integer ageYear,
		String growthStage, String placementType, Integer trayCount, Long bedZoneId, String currentLocation,
		Long createdOrchidGroupId, List<Long> createdOrchidGroupIds, String worker, String memo,
	...
	public static InboundRecordResponse from(InboundRecord record) {
		return new InboundRecordResponse(record.getId(), record.getInboundDate(), record.getInboundType().name(),
				...
				record.getStatus().name(), record.getBottleCount(), record.getEstimatedQuantity(),
				record.getActualQuantity(), record.getTempLocation(), record.getPottingDueDate(),
				record.getPottingDate(), record.getPotSize(), record.getAgeYear(), record.getGrowthStage(),
				record.getPlacementType(), record.getTrayCount(),
	...
	private static String formatLocation(BedZone bedZone, String tempLocation) {
		if (bedZone != null) {
			return "%d동-%d다이 %s".formatted(bedZone.getPhysicalBed().getHouse().getNumber(),
					bedZone.getPhysicalBed().getNumber(), bedZone.getName());
		}
		return tempLocation;
	}
}
