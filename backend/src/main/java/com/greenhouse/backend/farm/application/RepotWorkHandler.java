...
public class RepotWorkHandler implements WorkEffectHandler {
	...
	public WorkExecutionResult execute(
		...
		if (source.getVariety() == null) {
			throw new IllegalArgumentException("품종이 연결되지 않은 난 묶음은 분갈이할 수 없습니다.");
		}
		...
		List<OrchidGroup> results = request.results().stream().map(row -> {
			...
			lineageService.record(
					source, result, OrchidGroupLineageRelationType.REPOTTED_TO, operation,
					request.inputQuantity(), row.quantity());
		...
		return new WorkExecutionResult("REPOT", details, results.stream().map(OrchidGroup::getId).toList());
	}
