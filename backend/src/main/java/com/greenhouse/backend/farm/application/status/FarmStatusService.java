...
public class FarmStatusService {
	...
	public FarmStatusMapResponse getMap() {
		...
		var groupsByHouseId = mapOrchidGroups.stream().collect(Collectors.groupingBy(group -> group.getHouseId()));
		var bedsByHouseId = physicalBedRepository.findAllInFarmOrder()
			.stream()
		...
		var houses = houseRepository.findAll()
			...
			.map(house -> {
				...
				return new HouseStatusSummaryResponse(house.getId(), house.getNumber(), house.getName(),
						houseGroups.size(),
						houseGroups.stream()
							.filter(group -> OrchidGroupStatusPolicy.isWarning(group.getStatus()))
							.count(),
						0, null,
						bedsByHouseId.getOrDefault(house.getId(), List.of())
							.stream()
	...
	public OrchidManagementViewportResponse getOrchidManagementViewport(Long startBedId, int bedCount) {
		...
		if (allBedRows.isEmpty()) {
			return new OrchidManagementViewportResponse(null, bedCount, List.of(), false, false,
		...
		return new OrchidManagementViewportResponse(visibleBedRows.getFirst().id(), bedCount,
				visibleBeds.stream().map(bed -> PhysicalBedResponse.from(bed, groupsByZoneId, businessDate)).toList(),
				startIndex > 0, startIndex + bedCount < allBedRows.size(),
				new OrchidManagementSummaryResponse(orchidGroupCount, totalQuantity, abnormalCount, bedZoneCount));
	}

	public List<OrchidManagementBedOrderResponse> getOrchidManagementBedOrder() {
		return physicalBedRepository.findAllOrderRows()
			.stream()
			.map(row -> new OrchidManagementBedOrderResponse(row.id(), row.houseId(), row.houseNumber(), row.number()))
			.toList();
	}
	...
	public FarmStatusZoomResponse getZoom(FarmZoomLevel level, Long houseId, Long physicalBedId) {
		...
		return switch (level) {
			...
			case BED_ZONE -> {
				...
				var zones = physicalBed.getBedZones()
					.stream()
					.map(zone -> BedZoneResponse.from(zone, groupsByZoneId.getOrDefault(zone.getId(), List.of()),
							businessDate))
					.toList();
				yield new FarmStatusZoomResponse(level, physicalBed.getHouse().getId(),
						physicalBed.getHouse().getNumber(), List.of(), zones);
			}
		};
	}

	private Map<Long, List<OrchidGroup>> loadGroupsByZoneId(List<Long> physicalBedIds) {
	...
	private List<OrchidGroup> searchOrchidGroupsByTarget(FarmStatusTargetType targetType, Long targetId) {
		return switch (targetType) {
	...
	private FarmStatusMapOrchidGroupResponse mapGroup(OrchidGroupRepository.MapRow row,
		...
		return new FarmStatusMapOrchidGroupResponse(row.getOrchidGroupId(), row.getHouseId(), row.getPhysicalBedId(),
				row.getBedZoneId(), row.getStartPosition(), row.getEndPosition(), row.getVarietyId(),
				row.getVarietyColor(), row.getVarietyName(), row.getQuantity(), row.getStatus(),
				OrchidGroupResponse.calculateAgeYear(row.getAgeYear(), referenceDate, businessDate), row.getPotSize(),
				row.getSortOrder());
	}
