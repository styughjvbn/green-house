...
public record AuctionSettlementResponse(
	...
	public static AuctionSettlementResponse from(AuctionSettlement settlement, String auctionHouseName, Map<Long, Result> results) {
		return new AuctionSettlementResponse(
				...
				settlement.getLines().stream().map(line -> AuctionSettlementLineResponse.from(
						line, results.get(line.getAuctionResultLineId()))).toList());
	}
}
