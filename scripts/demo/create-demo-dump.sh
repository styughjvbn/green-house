...
main() {
  ...
  pg_dump "${SANITIZE_DB_URL}" --format=custom --no-owner --no-privileges \
    --schema=public --schema=demo_internal --file="${output}"
  pg_restore --list "${output}" >/dev/null
  pg_restore --list "${output}" | grep -E 'TABLE DATA public flyway_schema_history' >/dev/null \
    || fail "Sanitized dump does not include flyway_schema_history data"
  (
    cd "$(dirname "${output}")"
