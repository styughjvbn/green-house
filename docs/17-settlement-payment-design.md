...
2026-07-06 기준 1~3단계 중 다음 항목을 구현했다.
...
- partner_payment_events, partner_balance_summaries 모델
- 경매 정산과 일반 판매전표 수동 입금 확인
- 부분입금·완납 상태와 입금 이력 조회
- 일반 판매전표 예상 입금일과 거래처 미수금 요약
...
설계와 현재 구현의 차이: 경매장 `rule_json`은 저장·편집만 가능하고 결과 자동 수신·파싱에는 아직 연결되지 않았다.

3단계에서는 입금 잔액을 초과하는 금액을 허용하지 않는다. 초과분을 예치금으로 전환하는 흐름과 `credit_balance` 갱신은 4단계에서 구현한다. 자동 매칭용 JSON 원본·점수·배정 정보도 5단계 전까지 비워 둔다.
