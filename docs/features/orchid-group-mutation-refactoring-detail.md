...
초기 검토에서는 재현하지 않은 위험이었으나, 2026-09-07 후속 검증에서 ACTIVE 다중 생성과 결과 2개의 포트 작업이 쓰기 context 없이 자동 flush되어 실패하는 것을 확인했다. 생성 경로가 그룹을 저장하기 전에 Mutation header와 트랜잭션 context를 설정하도록 수정했다. 실제 PostgreSQL의 다중 생성·재실행·다중 포트와 기존 단건 수정·폐기 롤백 검증을 통과했다. 결과별 배치 조회 최적화는 별도 후속 범위다.
