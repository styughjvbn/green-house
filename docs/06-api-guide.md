...
기간 작업 실행 API는 난 묶음 범위의 일반 기록형 작업과 자리 이동·분갈이 계획, 입고 기록 범위의 포트 작업 계획을 지원한다. 대상 미리보기, 생성, 기간·상태·범위별 목록, 상세, 대상별 진행·실행·건너뛰기, 작업 시작·일시중지·재개·취소·완료, 난 묶음 통합 이력을 제공한다.
...
자리 이동·분갈이·입고 포트 작업은 계획 생성 시 대상을 스냅샷으로 확정하되 위치나 구조를 변경하지 않는다. 자리 이동은 대상 완료 시 목적 구역·구간을 반영하고, 분갈이·포트 작업은 결과 위치·수량을 입력해 난 묶음 생성과 계보·효과 감사 기록을 같은 트랜잭션에서 처리한다. 다중 생성은 대상 없는 즉시 구조 변경 API로 유지한다.
