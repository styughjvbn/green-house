...
- 입금 이벤트와 경매 정산 목록은 root와 정산 행을 조회한 뒤 거래처 ID를 모아 Partner application API로 이름을 일괄 조회한다. 정산 행의 출하일·품종·등급은 Auction application의 결과 값으로 일괄 조립한다. 거래처 이름은 현재 기준 정보이며, 원장의 금액·입금자·날짜나 기존 정산 행의 보존된 값은 다시 계산하지 않는다.
